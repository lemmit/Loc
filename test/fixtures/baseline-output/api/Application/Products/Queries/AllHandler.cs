...
public sealed class AllHandler : IQueryHandler<AllQuery, Paged<ProductResponse>>
{
    ...
    public async ValueTask<Paged<ProductResponse>> Handle(AllQuery query, CancellationToken cancellationToken)
    {
        ...
        return new Paged<ProductResponse>(domain.Items.Select(d => new ProductResponse(d.Id.Value, d.Sku, new MoneyResponse((double)d.Price.Amount, d.Price.Currency), d.Version, d.Display)).ToList(), domain.Page, domain.PageSize, domain.Total, domain.TotalPages);
    }
}
