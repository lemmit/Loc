// M-T4.4 slice 7b — RabbitMQ queue semantics on the .NET backend, end to
// end.
//
// The all-dotnet sibling of channels-e2e-rabbit.test.ts: boots the GENERATED
// ASP.NET Core producer plus TWO REPLICAS of the generated consumer against
...
//
// Opt-in: LOOM_CHANNELS_E2E_RABBIT_DOTNET=1 (npm run
// test:channels-rabbit-dotnet).  Needs docker + the .NET SDK on PATH.
...
const ENABLED = process.env.LOOM_CHANNELS_E2E_RABBIT_DOTNET === "1";
...
const SALES_PORT = 3208;
const REPLICA_PORTS = [3209, 3210];
const PG_PORT = 55442;
const AMQP_PORT = 55674;
const ORDERS = 6;
...
describe.skipIf(!ENABLED)("rabbitmq queue semantics — dotnet leg (M-T4.4 slice 7b)", () => {
  let dir: string;
  ...
  const boot = (app: string, port: number, db: string): void => {
    ...
    const child = spawn("dotnet", ["run", "--no-restore", "--no-launch-profile"], {
      ...
      env: {
        ...process.env,
        ConnectionStrings__Default: pgConn(db),
  ...
  beforeAll(async () => {
    dir = mkdtempSync(join(tmpdir(), "loom-channels-e2e-rbdn-"));
    ...
    if (pgOverride) {
      ...
      sh(
        `docker run -d --rm --name loom-channels-rbdn-pg -e POSTGRES_PASSWORD=postgres -p ${PG_PORT}:5432 postgres:18-alpine`,
      );
      dockerNames.push("loom-channels-rbdn-pg");
      pgConn = (db) =>
      ...
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-rbdn-pg psql -U postgres -c "CREATE DATABASE sales_api;" -c "CREATE DATABASE ship_api;"`,
          );
          return true;
        },
        60_000,
    ...
    if (amqpOverride) {
      ...
      sh(
        `docker run -d --name loom-channels-rbdn-mq -p ${AMQP_PORT}:5672 --entrypoint sh rabbitmq:4-management-alpine ` +
      ...
      dockerNames.push("loom-channels-rbdn-mq");
      amqpUrl = `amqp://guest:guest@localhost:${AMQP_PORT}`;
      dlqProbe = true;
      await waitFor(
        async () => {
          sh(`docker exec loom-channels-rbdn-mq rabbitmq-diagnostics -q ping`);
          return true;
        },
        120_000,
    ...
    // Replica 1 boots first and owns the schema bootstrap; replica 2 joins
    // after (concurrent first-boot DDL races the migrator — a real-deployment
  ...
  it("parks a poisoned message in the DLQ instead of losing it", async () => {
    ...
    sh(
      `docker exec loom-channels-rbdn-mq rabbitmqadmin publish message --exchange ${ADDRESS} --payload 'not-json{{'`,
    );
    await waitFor(
      async () => {
        const out = sh(
          `docker exec loom-channels-rbdn-mq rabbitmqadmin get messages --queue loom.dlq.${ADDRESS}`,
        );
