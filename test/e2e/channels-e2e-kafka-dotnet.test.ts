// M-T4.4 slice 8b — Kafka log semantics on the .NET backend, end to end.
//
// Boots the GENERATED ASP.NET Core producer plus TWO REPLICAS of the generated
...
//      keeps moving.
//
// Opt-in: LOOM_CHANNELS_E2E_KAFKA_DOTNET=1 (npm run test:channels-kafka-dotnet).  Needs
...
const ENABLED = process.env.LOOM_CHANNELS_E2E_KAFKA_DOTNET === "1";
...
const SALES_PORT = 3223;
const REPLICA_PORTS = [3224, 3225];
const PG_PORT = 55447;
const KAFKA_PORT = 55679;
const ORDERS = 6;
...
function consumedEntries(log: string): { type: string; key: string }[] {
  ...
  for (const line of log.split("\n")) {
    ...
    // .NET's JSON console logger capitalizes the structured property names.
    const type = /"[tT]ype":\s*"([^"]+)"/.exec(line)?.[1];
    const key = /"[kK]ey":\s*"([^"]+)"/.exec(line)?.[1];
...
describe.skipIf(!ENABLED)("kafka log semantics — dotnet leg (M-T4.4 slice 8b)", () => {
  ...
  const boot = (app: string, port: number, db: string): void => {
    // `dotnet run` spawns the server as a grandchild — detached process group
    // so teardown can kill the whole tree (see channels-e2e-dotnet.test.ts).
    const child = spawn("dotnet", ["run", "--no-restore", "--no-launch-profile"], {
      cwd: join(dir, "out", app),
      detached: true,
      env: {
        ...process.env,
        ConnectionStrings__Default: pgUrl(db),
  ...
  beforeAll(async () => {
    dir = mkdtempSync(join(tmpdir(), "loom-channels-e2e-kfdn-"));
    ...
    if (pgOverride) {
      ...
      pgUrl = (db) =>
        `Host=${u.hostname};Port=${u.port || "5432"};Database=${db};Username=${u.username || "postgres"};Password=${u.password || "postgres"}`;
    } else {
      sh(
        `docker run -d --rm --name loom-channels-kfdn-pg -e POSTGRES_PASSWORD=postgres -p ${PG_PORT}:5432 postgres:18-alpine`,
      );
      dockerNames.push("loom-channels-kfdn-pg");
      pgUrl = (db) =>
      ...
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-kfdn-pg psql -U postgres -c "CREATE DATABASE sales_api;" -c "CREATE DATABASE ship_api;"`,
          );
          return true;
        },
        60_000,
    ...
    if (kafkaOverride) {
      ...
      sh(
        `docker run -d --name loom-channels-kfdn-kafka -p ${KAFKA_PORT}:19092 ` +
      ...
      dockerNames.push("loom-channels-kfdn-kafka");
      kafkaUrl = `localhost:${KAFKA_PORT}`;
      dlqProbe = true;
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-kfdn-kafka /opt/kafka/bin/kafka-broker-api-versions.sh --bootstrap-server localhost:9092`,
          );
          return true;
        },
        120_000,
    ...
    for (const app of ["sales_api", "ship_api"] as const) {
      sh("dotnet restore", join(dir, "out", app));
    }
  ...
  it("parks a poisoned record on <address>.dlq instead of stalling the partition", async () => {
    ...
    sh(
      `docker exec loom-channels-kfdn-kafka sh -c 'echo "not-json{{" | /opt/kafka/bin/kafka-console-producer.sh --bootstrap-server localhost:9092 --topic ${ADDRESS}'`,
    );
    await waitFor(
      async () => {
        const out = sh(
          `docker exec loom-channels-kfdn-kafka sh -c '/opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic ${ADDRESS}.dlq --from-beginning --timeout-ms 5000 2>/dev/null || true'`,
        );
