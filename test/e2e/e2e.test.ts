...
describe.skipIf(!RUN)("e2e: docker compose smoke", () => {
  ...
  it("cross-backend: a guarded workflow denies with 403 (runtime authorization)", async () => {
    ...
    // fails for any authenticated request — which exercises the
    // authorization (403) path end-to-end on all three backends:
    //   - Hono   maps ForbiddenError      → 403
    //   - .NET   maps ForbiddenException  → 403
    //   - Phoenix maps {:error, :forbidden} → 403  (was a hardcoded 400)
    // The body is valid (name + visibility) so Hono/.NET request validation
    // doesn't short-circuit with a 400 before the guard runs.  The stub
    // accepts any bearer token, so a placeholder satisfies `auth: required`.
    ...
    // Every backend must deny with 403 — not 400 (domain/validation) and not
    // 500 (the pre-fix Phoenix ArgumentError path).  Asserting all three at
    // once surfaces which backend diverged in the failure message.
