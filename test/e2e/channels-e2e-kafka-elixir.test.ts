// M-T4.4 slice 8d — Kafka log semantics on the Phoenix backend, end to end.
//
// The all-elixir sibling of channels-e2e-kafka-java.test.ts, booted the
// channels-e2e-rabbit-elixir way (host `mix phx.server`, MIX_ENV=dev):
// the GENERATED Phoenix producer plus TWO REPLICAS of the generated Phoenix
// consumer against docker postgres + apache/kafka (KRaft) sidecars, proving
// the log contract on this backend:
//   1. exactly-one-of-N within the deployable's group — every emit lands in
//      `__loom_outbox` via the tee, is published by the OutboxRelay with the
//      row id as the envelope id (design §5) through brod's :hash
//      partitioner, and is consumed by exactly one replica of the group;
...
//      keeps moving.
//
// Opt-in: LOOM_CHANNELS_E2E_KAFKA_ELIXIR=1 (npm run
// test:channels-kafka-elixir).  Needs docker + `mix` (Elixir 1.16+) on PATH.
// LOOM_CHANNELS_PG_URL / LOOM_CHANNELS_KAFKA_URL point at existing services
// (the DLQ leg is docker-only and skips under a kafka override).
...
const SALES_PORT = 3229;
const REPLICA_PORTS = [3230, 3231];
const PG_PORT = 55449;
const KAFKA_PORT = 55681;
const ORDERS = 6;
...
describe.skipIf(!ENABLED)("kafka log semantics — elixir leg (M-T4.4 slice 8d)", () => {
  ...
  beforeAll(async () => {
    dir = mkdtempSync(join(tmpdir(), "loom-channels-e2e-kfex-"));
    ...
    if (pgOverride) {
      ...
      sh(
        `docker run -d --rm --name loom-channels-kfex-pg -e POSTGRES_PASSWORD=postgres -p ${PG_PORT}:5432 postgres:18-alpine`,
      );
      dockerNames.push("loom-channels-kfex-pg");
      ...
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-kfex-pg psql -U postgres -c "CREATE DATABASE sales_api;" -c "CREATE DATABASE ship_api;"`,
          );
          return true;
        },
        60_000,
    ...
    if (kafkaOverride) {
      ...
      sh(
        `docker run -d --name loom-channels-kfex-kafka -p ${KAFKA_PORT}:19092 ` +
      ...
      dockerNames.push("loom-channels-kfex-kafka");
      kafkaUrl = `localhost:${KAFKA_PORT}`;
      dlqProbe = true;
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-kfex-kafka /opt/kafka/bin/kafka-broker-api-versions.sh --bootstrap-server localhost:9092`,
          );
          return true;
        },
        120_000,
  ...
  it("parks a poisoned record on <address>.dlq instead of stalling the partition", async () => {
    ...
    sh(
      `docker exec loom-channels-kfex-kafka sh -c 'echo "not-json{{" | /opt/kafka/bin/kafka-console-producer.sh --bootstrap-server localhost:9092 --topic ${ADDRESS}'`,
    );
    await waitFor(
      async () => {
        const out = sh(
          `docker exec loom-channels-kfex-kafka sh -c '/opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic ${ADDRESS}.dlq --from-beginning --timeout-ms 5000 2>/dev/null || true'`,
        );
