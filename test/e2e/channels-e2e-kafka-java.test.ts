// M-T4.4 slice 8c — Kafka log semantics on the Java backend, end to end.
//
// Boots the GENERATED Spring Boot producer plus TWO REPLICAS of the generated
...
//      keeps moving.
//
// Opt-in: LOOM_CHANNELS_E2E_KAFKA_JAVA=1 (npm run test:channels-kafka-java).  Needs
// docker + `gradle` (9.1+) + JDK 25 on PATH.  LOOM_CHANNELS_PG_URL /
...
const ENABLED = process.env.LOOM_CHANNELS_E2E_KAFKA_JAVA === "1";
...
const SALES_PORT = 3226;
const REPLICA_PORTS = [3227, 3228];
const PG_PORT = 55448;
const KAFKA_PORT = 55680;
const ORDERS = 6;
...
describe.skipIf(!ENABLED)("kafka log semantics — java leg (M-T4.4 slice 8c)", () => {
  let dir: string;
  ...
  const boot = (app: string, port: number, db: string): void => {
    ...
    const child = spawn(javaBin, ["-jar", join("build", "libs", jarOf(app))], {
      ...
      env: {
        ...process.env,
        SPRING_DATASOURCE_URL: pgUrl(db),
  ...
  beforeAll(async () => {
    dir = mkdtempSync(join(tmpdir(), "loom-channels-e2e-kfjv-"));
    ...
    if (pgOverride) {
      ...
      sh(
        `docker run -d --rm --name loom-channels-kfjv-pg -e POSTGRES_PASSWORD=postgres -p ${PG_PORT}:5432 postgres:18-alpine`,
      );
      dockerNames.push("loom-channels-kfjv-pg");
      pgUrl = (db) => `jdbc:postgresql://localhost:${PG_PORT}/${db}`;
      // The postgres entrypoint restarts after initdb — make CREATE DATABASE
      // itself the retried probe (see channels-e2e.test.ts).
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-kfjv-pg psql -U postgres -c "CREATE DATABASE sales_api;" -c "CREATE DATABASE ship_api;"`,
          );
          return true;
        },
        60_000,
    ...
    if (kafkaOverride) {
      ...
      sh(
        `docker run -d --name loom-channels-kfjv-kafka -p ${KAFKA_PORT}:19092 ` +
      ...
      dockerNames.push("loom-channels-kfjv-kafka");
      kafkaUrl = `localhost:${KAFKA_PORT}`;
      dlqProbe = true;
      await waitFor(
        async () => {
          sh(
            `docker exec loom-channels-kfjv-kafka /opt/kafka/bin/kafka-broker-api-versions.sh --bootstrap-server localhost:9092`,
          );
          return true;
        },
        120_000,
  ...
  it("parks a poisoned record on <address>.dlq instead of stalling the partition", async () => {
    ...
    sh(
      `docker exec loom-channels-kfjv-kafka sh -c 'echo "not-json{{" | /opt/kafka/bin/kafka-console-producer.sh --bootstrap-server localhost:9092 --topic ${ADDRESS}'`,
    );
    await waitFor(
      async () => {
        const out = sh(
          `docker exec loom-channels-kfjv-kafka sh -c '/opt/kafka/bin/kafka-console-consumer.sh --bootstrap-server localhost:9092 --topic ${ADDRESS}.dlq --from-beginning --timeout-ms 5000 2>/dev/null || true'`,
        );
