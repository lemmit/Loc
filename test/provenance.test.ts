...
async function parseModel(src: string): Promise<{ model: Model; errors: string[]; warnings: string[] }> {
...
describe("provenanced — IR lowering", () => {
  ...
  it("derives a content-addressed, stable snapshotId (same rule ⇒ same id)", async () => {
    ...
    const idOf = (loom: typeof a) =>
      loom.systems[0]!.modules[0]!.contexts[0]!.aggregates[0]!.operations
        .find((o) => o.name === "applyTotal")!
...
function findProperty(model: Model, agg: string, field: string): Property {
  for (const node of AstUtils.streamAst(model)) {
    if (
      isProperty(node) &&
      node.name === field &&
      AstUtils.getContainerOfType(node, (n): n is import("../src/language/generated/ast.js").Aggregate =>
        n.$type === "Aggregate",
      )?.name === agg
    ) {
      return node;
