...
import type { Aggregate, EventDecl, Model, Property, ValueObject } from "../src/language/generated/ast.js";
import {
  addField,
  availableTypes,
  deleteField,
...
describe("System builder — inline field editing", () => {
  it("adds a primitive / array / optional field to an aggregate", () => {
    let src = sales;
    src = addField(src, "aggregate", "Order", "qty", { base: { kind: "primitive", name: "int" }, array: false, optional: false })!;
    src = addField(src, "aggregate", "Order", "tags", { base: { kind: "primitive", name: "string" }, array: true, optional: false })!;
    src = addField(src, "aggregate", "Order", "note", { base: { kind: "primitive", name: "string" }, array: false, optional: true })!;
    expect(src).not.toBeNull();

    const order = find<Aggregate>(parse(src), "Aggregate", "Order");
    const props = Object.fromEntries(
      (order.members.filter((m) => m.$type === "Property") as Property[]).map((p) => [p.name, p.type]),
    );
  ...
  it("deletes a field, leaving the rest intact", () => {
    ...
    expect(after.map((f) => f.name)).toEqual(before.filter((f) => f.name !== "currency").map((f) => f.name));
  ...
  it("edits an event's fields too", () => {
    ...
    expect(ev.fields.some((f) => f.name === "at" && f.type.base.$type === "PrimitiveType")).toBe(true);
  });
