...
// domain bodies (Duration algebra, `Duration.between` for dt−dt) AND
// where-position lowering: HQL duration arithmetic (`e.dueDate + 30 day`) in
// the JPQL `@Query` finds plus HibernateCriteriaBuilder duration arithmetic
// (`addDuration` / `subtractDuration` / `durationScaled`) in the reified
// criterion Specifications.  Runtime representation: an absolute duration is
// a `java.time.Duration`.  Boot-verified end-to-end against Postgres (Spring
// parses every @Query at startup; the finds and Specifications executed with
// correct row sets).
...
describe("java generator — A5 temporal", () => {
  ...
  it("lowers where-position datetime ± duration on the VALUE side too (all three units typed)", async () => {
    const jpa = (await files()).get(`${ROOT}/features/invoices/InvoiceJpaRepository.java`)!;
