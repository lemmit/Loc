...
describe("java generator — wire validators + advice (S5)", () => {
  it("emits a unified Spring Validator per command, run at the @Valid seam", async () => {
    ...
    expect(v).toContain(
      'if (!(code.codePointCount(0, code.length()) >= 1)) errors.rejectValue("code", "loom.invariant", "Invariant violated: code.length > 0");',
    );
...
`;
    const out = await generateSystemFiles(src);
    const check =
      'if (!(handle.codePointCount(0, handle.length()) >= 1)) errors.rejectValue("handle", "loom.invariant", "Invariant violated: handle.length > 0");';
