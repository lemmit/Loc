...
system Helpdesk {
  ...
  deployable web { platform: react targets: api ui: WebApp { Support: api } port: 3001${authUi ? " auth: ui" : ""} }
}
`;

describe("React default sidebar honours the list page's requires gate", () => {
  it("wraps the default aggregate nav entry in the gate when auth: ui", async () => {
    ...
    const shell = find(files, "web/src/App.tsx");
    expect(shell).toContain('currentUser.role === "agent"');
    expect(shell).toContain('data-testid="nav-tickets"');
    expect(shell).toContain('{(currentUser.role === "agent") ? <NavLink');
  });
