...
describe("Phoenix typed in-system api client", () => {
  ...
  it("returns the callee's ABSENCE UNION as a value, not a throw", async () => {
    ...
    const src = files.get("shipping_svc/lib/shipping_svc/resources/api_clients.ex") ?? "";
    expect(src).toContain("if res.status == 404 do");
    expect(src).toMatch(/if res\.status == 404 do\n\s*nil\n\s*else/);
  });
