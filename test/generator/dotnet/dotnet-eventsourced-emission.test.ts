...
describe(".NET event-sourcing emission (persistedAs(eventLog))", () => {
  it("emits the EF event-record entity + config (no normalised entity config)", async () => {
    const files = await generate();
    const keys = [...files.keys()];
    expect(keys).toContain("Infrastructure/Persistence/Events/AccountEventRecord.cs");
  ...
  it("emits the event-store repository (fold on load, append on save)", async () => {
    ...
    expect(repo).toContain("_db.AccountEvents.Add(new AccountEventRecord");
    expect(repo).toContain('"Opened" => System.Text.Json.JsonSerializer.Deserialize<Opened>');
    // The DbContext exposes the event-record DbSet.
    const ctx = (await generate()).get("Infrastructure/Persistence/AppDbContext.cs")!;
    expect(ctx).toContain("DbSet<AccountEventRecord> AccountEvents");
  });
