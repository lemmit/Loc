...
describe(".NET showcase compile regressions (conformance-parity breakers)", () => {
  ...
  it("variant-match over a union-find binding renders the null-check switch, not carrier patterns", async () => {
    const handler = find(await build(), "Application/Workflows/ResolveProjectHandler.cs");
    expect(handler).toContain("{ } p => p.Name");
    expect(handler).toContain('null => "not found"');
  ...
  it("seed rows fill omitted create-inputs and parse datetime literals", async () => {
    ...
    expect(seed).toContain(
      'Project.Create("Alpha", null, true, null, DateTime.Parse("2024-01-01T00:00:00Z", CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal))',
    );
  ...
  it("the create controller unwraps an optional id-ref wire member with .Value", async () => {
    ...
    expect(ctrl).toContain("(request.Parent is null ? null : new ProjectId(request.Parent.Value))");
...
describe(".NET AuditableInterceptor — principal-member stamp values", () => {
  it("materialises the ambient currentUser local for a `currentUser.<member>` stamp", () => {
    ...
    // The stamp value renders as a member read on a real local, materialised
    // from the ambient RequestContext inside a BRACED arm (switch-section
    // locals share the switch block's declaration space).
    expect(out).toContain("var currentUser = RequestContext.Current!.CurrentUser!;");
    expect(out).toContain(
      "ctx.Entry(e).Property(x => x.CreatedByRole).CurrentValue = currentUser.Role;",
    );
