...
describe("seed datetime string literals coerce to real datetime values", () => {
  ...
  it("hono: the db:seed direct-run guard cannot misfire inside the tsup bundle", async () => {
    // db/seed.ts is BUNDLED into dist/index.js, where `import.meta.url` IS
    // the executed file — a bare URL-equality guard fired at import time and
    // a second unawaited seeder raced the app's own migrate+seed boot (the
    // conformance-parity ECONNREFUSED on :3000).  The guard must also require
    // the executed file to actually BE the seed module.
    const seed = find(await build("node", 3000), "db/seed.ts");
    expect(seed).toContain("/seed\\.(ts|mts|js|mjs)$/.test(import.meta.url)");
  });
