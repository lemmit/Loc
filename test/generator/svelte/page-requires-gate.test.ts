// Page `requires` UI gate on Svelte (D-AUTH-OIDC, UI gate).  A
// `page X { requires <expr> ... }` on a SvelteKit frontend with `auth: ui`
// binds the verified session user in `<script>` and wraps the body in an
// `{#if}` that renders a `<Forbidden/>` fallback when the currentUser-only
// predicate fails — the client mirror of the backend 403.  Without `auth: ui`
// (or without a gate) the page is byte-identical to before.
...
describe("svelte page requires gate", () => {
  it("wraps the body in an {#if} Forbidden guard against the session user", async () => {
    const files = await generateSystemFiles(
      SYS({ authUi: true, gate: 'requires currentUser.role == "agent"\n      ' }),
    );
    const page = find(files, "secret/+page.svelte");
    expect(page).toContain('import { useSession } from "$lib/auth/AuthGate.svelte";');
