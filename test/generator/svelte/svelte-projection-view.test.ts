// Shorthand projection-sourced views on Svelte (projection.md v1.1): the svelte
// views api module (createQuery flavor) emits a shorthand projection view's
// `<View>Row` zod schema INLINE from the projection's wire shape — no
// `<Proj>ListResponse` import to re-export (a projection has no frontend api
// module).
//
// Svelte sibling of test/generator/react/react-projection-view.test.ts.

import { describe, expect, it } from "vitest";
import { buildViewsApiModule } from "../../../src/generator/svelte/view-builder.js";
...
describe("Svelte shorthand projection-sourced view api module", () => {
  it("emits the row schema inline from the projection wire shape (createQuery)", async () => {
    ...
    // createQuery flavor of the read hook.
    expect(mod).toContain("export function useShippedOrdersView() {");
    expect(mod).toContain("return createQuery(() => ({");
