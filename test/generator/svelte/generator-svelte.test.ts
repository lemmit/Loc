...
describe("svelte generator — project shape", () => {
  ...
  it("walks page bodies through the shared walker with the svelte target", async () => {
    ...
    // Since M-T1.1 the scaffold list sorts client-side, so the rows expr is
    // wrapped in `sortRows(...)`; the `(… ?? [])` guard still surrounds it.
    expect(list).toContain("{#each (sortRows(customerAll.data, sortKey, sortDir) ?? []) as row (row.id)}");
