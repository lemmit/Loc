  ...
  it("an elixir host accepts a react ui (static bundle on the static-asset host)", async () => {
    const { errors } = await parseString(`
      system S {
        ...
        deployable app { platform: elixir { foundation: ash }, contexts: [C], hosts: Web, port: 4000 }
      }
    `);
    expect(
      errors.some((e) => UNHOSTABLE.test(e)),
  ...
  it("an elixir host with the coreComponents pack passes the design-pack format check", async () => {
    const { errors } = await parseString(`
      system S {
        ...
        deployable app { platform: elixir { foundation: ash }, contexts: [C], hosts: Admin, port: 4000, design: coreComponents }
      }
    `);
    expect(
