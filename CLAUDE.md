...
npm run test:k8s-e2e      # cluster smoke — install the emitted chart into a kind cluster + throwaway postgres, assert workloads boot + backend /ready + a real findAll GET round-trips (migrated table → wire shape); needs kind + kubectl + helm + docker (run `kind create cluster` first)
```
...
- `k8s-e2e.yml` — heavier cluster smoke: installs the emitted chart into a `kind` cluster (+ throwaway postgres) and asserts the workloads boot, the backend reaches `/ready`, and a real auto-`findAll` `GET` (discovered from `/openapi.json`) round-trips through the migrated DB. NOT per-PR — runs nightly, on the `e2e-k8s` PR label, or via manual dispatch. See `docs/kubernetes.md`.
