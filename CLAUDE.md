...
The framework-specific seams (state read/write syntax, navigation, `match` rendering, default-init) are framework-shaped and cannot be expressed as pack templates. `src/generator/_walker/target.ts` **defines** the `WalkerTarget` contract; `src/generator/react/walker/tsx-target.ts` and `src/generator/phoenix-live-view/walker/heex-target.ts` are the concrete implementations both walkers consume via `WalkContext.target` (state read/write delegate to `target.stateRead` / `target.stateWrite`, navigation through `target.renderNavigate`, match through `target.renderMatch`). A few seams that *could* fit the interface — helper imports and API-call lowering — stay inlined because the cross-walker call shapes differ enough that a contract method would just wrap each platform's existing path; see the deferred-seams comment in `target.ts` for the rationale. Acceptance gate for any change here is byte-identical fixture output.
