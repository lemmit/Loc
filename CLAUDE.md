...
| `designs/` | Design packs, 13 families with versioned subdirs (`designs/<family>/<vNN>/pack.json`; the `format` field names the framework, absent = React). **React:** `mantine` (v7, v9), `mui` (v5, v7), `chakra` (v2, v3), `shadcn` (v3, v4). **Vue:** `vuetify` (v3), `shadcnVue` (v1). **Svelte:** `flowbite` (v1), `shadcnSvelte` (v1). **Angular:** `angularMaterial` (v1), `primeng` (v1), `spartanNg` (v1). **Phoenix HEEx** (`format:"heex"`, `.heex.hbs` templates): `coreComponents` (v3, the baseline) and `daisyui` (v1) — these replaced the old `ashPhoenix` pack. Each React/Vue/Svelte/Angular pack is a tree of templates that the body-walker dispatches into. Feliz and Flutter have no `.hbs` pack pipeline (they self-host). **The two HEEx packs are currently INERT** — the elixir generator makes no `pack.render` call at all (its markup comes from the procedural `heex-primitives.ts` specs and its shell from `elixir/shell/web.ts`), so every `.heex.hbs` file is unrendered and `design:` on a `platform: elixir` deployable is a silent no-op: generating one system under `design: daisyui` vs `design: coreComponents` yields byte-identical output. The `app-shell` required-emit (`_packs/required-primitives.ts` → `SHARED_SHELL`) still demands the files exist, which is what keeps them alive. Editing a HEEx pack template changes nothing — fix the emitter, not the pack. |
...
**Which gate catches what — and why `main` still goes red.** Branch protection requires only `tests passed` (the fast vitest rollup — job id `tests-passed`, check name `tests passed`); every heavy runtime/boot gate (`tenancy-e2e`, the `*-obs-e2e`, the `*-oidc-e2e`, `auth-oidc-compose-e2e`, `pages`, the deploy builds) is *non-required* and many don't run on a PR at all — so a change can be green on its PR and break `main` one merge later. **The merge queue is the fix, and the workflow side of it is now done**: all 39 gates in the intended required set carry a `merge_group:` trigger and expose one stable check name (a `<stem>-passed` rollup where the workflow is a matrix), so only the repo-settings flip remains — the manifest is `test/system/merge-queue-required-checks.ts` and `test/system/merge-queue-readiness.test.ts` fails the fast suite if a workflow drifts out of readiness. [`docs/ci-gating.md`](docs/ci-gating.md) documents the tiers, the full check-name list, and the activation runbook; consult it before assuming a green PR means a green `main`.
