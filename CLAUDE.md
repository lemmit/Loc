...
npm run test:k8s-e2e      # cluster smoke — install the emitted chart into a kind cluster + throwaway postgres, assert workloads boot + backend /ready; needs kind + kubectl + helm + docker (run `kind create cluster` first)
```
...
- `k8s-e2e.yml` — heavier cluster smoke: installs the emitted chart into a `kind` cluster (+ throwaway postgres) and asserts the workloads boot + backend `/ready`. NOT per-PR — runs nightly, on the `e2e-k8s` PR label, or via manual dispatch. See `docs/kubernetes.md`.
