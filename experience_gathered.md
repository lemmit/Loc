...
- **`mix compile --warnings-as-errors` makes every warning a release
  ...
  block every PR touching the generator:
  ...
  2. `redefining module Inspect.<App>.<Ctx>.<Agg>`.  PR #537 emitted
     ...
     the resource struct; the two collide at compile time.  Fix in
     this branch: render the same expression body via
     **`String.Chars`** instead — semantic match for the .NET
     `ToString()` / TS `toString()` channel, and Ash's macro doesn't
     touch String.Chars.  IEx / Logger keep Ash's struct Inspect
     output; explicit `to_string(record)` (and string interpolation)
     produce the custom string.
  ...
  protocol surface.  Picking a protocol the upstream macro ignores
  (here: String.Chars vs Inspect) sidesteps the entire collision
  class.
