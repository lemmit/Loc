...
- Generate → Bundle → Boot → `GET /products` → `POST /products` → `GET /products`.
- The bundler fetches ~150 modules from `esm.sh`; the runtime fetches PGlite's WASM + `.data` from `jsdelivr`.
