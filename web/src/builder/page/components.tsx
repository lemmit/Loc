...
function renderLeaf(name: PrimitiveName, p: Props): ReactNode {
  switch (name) {
    ...
    case "CreateForm":
      return <span style={{ color: "var(--mantine-color-teal-4)" }}>▤ CreateForm{p.of ? ` of ${p.of}` : ""}</span>;
    case "OperationForm":
      return <span style={{ color: "var(--mantine-color-teal-4)" }}>▤ OperationForm{p.op ? ` ${p.op}` : p.operation ? ` ${p.operation}` : ""}</span>;
    case "WorkflowForm":
      return <span style={{ color: "var(--mantine-color-teal-4)" }}>▤ WorkflowForm{p.runs ? ` runs ${p.runs}` : ""}</span>;
    case "Stat":
