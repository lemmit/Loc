...
so `jq 'select(.event == "request_end") | {path, duration_ms, status}'` works unchanged on any deployable. One wrinkle on Elixir: the catalog `warn` level renders as `Logger.warning(...)` (Elixir's spelling), and the generated `LogFormatter` stringifies the level it is handed — so those lines carry `"level":"warning"` on the wire while every other backend writes `"warn"`. Match both when you filter across a polyglot stack.
