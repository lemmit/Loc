...
- **PR1 (contract-record layer) landed** — `scaffoldHandlers` now splices source-visible literal `response`/`command`/`query` `PayloadDecl` records (`src/macros/api/factories.ts` `payload`/`response`/`command`/`query` + `apiReadFields` = AST twin of `forApiRead(wireShape)`; `src/macros/stdlib/scaffold/_contracts-shared.ts`). Macro-layer only, additive + **inert** (byte-identical generation, proven). `unfold` ejects the contract as real `.ddd`.
- **Remaining (PR2+):** rewire the 5 explicit-handler emitters to READ the response record (+ override-by-name suppression in the DTO emitters, needed only once a backend reads it); handlers take `cmd`/`q` record params + declare `<Agg>Response` returns; tighten Hono 200 schema `z.unknown()` → `<Agg>Response`. Staged one backend per PR (wire-conformance-gated), .NET first.
