...
## M-T6.15 — Feliz silent-drop fallthroughs → fail-fast — `partial` · **S** · P1 ⭐ wrong failure mode
Both `default:` arms now **fail fast** instead of silently emitting compiling-but-wrong F#: `feliz/update-emit.ts` throws on any unhandled action statement kind (was `// TODO feliz update: <kind>` — control flow vanished; assign/add/remove/let still render), and `feliz/fs-expr.ts` throws on any unhandled expression kind (was `(* unsupported *) ()` unit — value discarded). Matches the Feliz backend's existing fail-fast idiom (`feliz/index.ts`) and the shared walker-core; pinned by `test/generator/feliz/fail-fast.test.ts`. **Remaining:** implement the kinds worth having — action-body `call`/`emit`/`variant-match` on the update path, and `method-call`/`match`/`lambda`/`new` on the F# expr path (each on its own schedule). Found by the 2026-07-13 hollow-work audit (M-T9.8).
