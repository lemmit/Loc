...
## M-T6.1 — Phoenix hosts SPA: wire the embed — `partial` · **L** · P1 ⭐ silent hole
**Phase 6 wired (PR #1889, real embed — not the interim gate).** The scaffolding on `main` was dead code (`renderSpaController` never called, `renderDockerfile`'s spa-build stage never flagged, `embedReact` only turned LiveView *off*); a `platform: elixir` host of a `framework: react|vue|svelte` ui therefore emitted a UI-less project. Now wired end-to-end in `elixir/vanilla/index.ts` + `vanilla/shell-emit.ts`: S2 SPA emit under `assets/` (dispatch on `uiFramework`, `{ apiBaseUrl: "/api", pathPrefix: "assets/", basePath: "/app" }`, merged via the prefix-generalised `embedSpaInto`), S3 endpoint `Plug.Static only: ~w(app)`, S4 router `/app` `SpaController` catch-all, S5 Dockerfile spa-build flag (svelte `build/` else `dist/`). No LiveView in embed mode. **Evidence:** `mix compile --warnings-as-errors` green on the `vanilla-embed-react.ddd` fixture (already in the `elixir-vanilla-build` matrix, now exercising the real path); the emitted React SPA vite-builds with the `/app/` base-href; JSON-API + LiveView paths byte-identical (diff over tasks-vanilla + showcase). **Remaining:** the embedded SPA's TS build is proven locally but not yet its own per-PR CI cell (the standalone react-build matrix already gates the same `generateReactForContexts` output); Phase 7 canonical `phoenixLiveView`→`phoenix` rename stays deferred. Flip to `done` once #1889 is green + merged.
