...
## M-T6.47 — The .NET decimal→double hops #2631 didn't reach — `done` (PR #2675, verified 2026-08-24) · **S–M** · P1
...
**Two corrections this mission produced.** (1) The audit's example is wrong: `0.30000000000000004` round-trips **exactly** through the CLR cast (`.NET 10.0.11`, measured) — the real story is that **9.20%** of doubles in `[0,100)` do not (3M-sample measurement on .NET 10.0.11), e.g. `99.52989333734583 → 99.52989333734584`. (2) #2631's "EF materializes `Average` as a real `double`" holds only for an **integral** column, which is what its `avg(o.lineCount)` fixture aggregated; the stale comment is corrected in code.

**Verified.** Emission suites extended to both new arms with the false case pinned too (int column keeps the plain cast, dapper grows no Parse); a "no emitted `double.Parse` shares a statement with a live `IQueryable`" scan over the whole emitted C# surface; `dotnet build /warnaserror` clean on a purpose-built witness project (efcore + dapper) and on the `projection-aggregation` (both adapters), `projection-groupby` and `core-domain` corpus fixtures; both arms mutation-proved by file-copy revert. **Not** done: the stored-17-digit runtime round-trip witness (no shared behavioral fixture carries a decimal column — F18's witness starvation).
