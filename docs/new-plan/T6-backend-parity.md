...
## M-T6.10 — Vanilla as a first-class adapter + `resolvePersistence()` — `partial` · **M** · P3
...
**`resolvePersistence()` RESOLVED BY REMOVAL 2026-07-13 (#1887, via M-T9.2).** The M-T9.2 divergence audit showed the *adapter*-registry granularity is the wrong seam for emission — the real cross-backend sharing is *sub-fragment* and crosses adapters (see [`missions/M-T9.2-persistence-seam-design.md`](missions/M-T9.2-persistence-seam-design.md) §2.5/§0.7), so *wiring* `resolvePersistence` would have wired the wrong abstraction. It had 0 production callers and was removed; its tests were rewired to the live `adaptersFor(...).persistence[name]` menu. **Remaining (this mission's residue):** the four dead `PersistenceAdapter.emit*` methods (`emitConnectionSetup`/`emitRepository`/`emitMigrations`/`emitOutbox`) — confirmed 0 prod callers but entangled with the pinned interface + the `stubAdapter` proxy + `contract-shape`/`stub-throws` tests; remove in a focused follow-up. (`emitProjectDeps` is LIVE — `hono/v4/emit.ts:808` — and stays.) The "route everything through the registry" alignment is superseded: the seam lives *inside* each backend's emitters, not behind the adapter registry.
