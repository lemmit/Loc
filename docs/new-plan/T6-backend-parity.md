...
## M-T6.23 — `persistence: mikroorm` non-persistence feature gaps — `partial` (all 5 original emitters landed; a SIXTH gate is live) · **M–L** · P2 ⭐ was silent

> **Named remainder (2026-08-17): the hierarchical-tenancy subtree predicate.** The status was flipped back from `done` because a `loom.mikroorm-unsupported` gate is still emitted — `validateMikroOrmSupport` refuses any aggregate carrying a `deep`/`global` tenancy scope filter (`src/ir/validate/checks/system-checks.ts:2995-3010`), and the register still carries the live `kind: "gap"` row for the code (`src/diagnostics/unsupported-register.ts:198-202`, owner `M-T6.23`). `whereToMikroFilter`'s FilterQuery subset cannot express the descendant-or-self predicate and *catches* the failure, leaving the filter unapplied — which is not a degraded read but no tenant predicate at all, so the gate is load-bearing. Closing this mission means making the subtree predicate expressible on the adapter and deleting both the gate and the register row (the ratchet in `test/system/unsupported-register.test.ts` fails on a stale row). The claim below that "`validateMikroOrmSupport`'s entire feature-gate family is gone" was true of the FIVE original features only.
