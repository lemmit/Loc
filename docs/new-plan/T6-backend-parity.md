...
## M-T6.15 — Feliz silent-drop fallthroughs → implemented / honestly gated — `partial` · **S→M** · P1 ⭐ wrong failure mode
...
- **Honestly gated at validation (not a codegen crash):** `loom.feliz-store-unsupported` (a store on a Feliz-hosted ui — the store subsystem needs Elmish Model/Msg composition + a store-scoped read seam) and `loom.feliz-async-effect-unsupported` (`match await` — the SPA variant-match async envelope has no Feliz renderer). Both in `store-checks.ts`.
...
Net: **zero silent drops and zero codegen crashes on valid `.ddd`** — every reachable construct is implemented or gated at validation. Tests: `test/generator/feliz/action-body.test.ts` (arms), `fail-fast.test.ts` (backstops), `test/ir/store.test.ts` (both gates). **Remaining (own missions):** build the Feliz store subsystem and the `match await` async-effect renderer (each needs a Fable-compile + runtime CI gate). Found by the 2026-07-13 hollow-work audit (M-T9.8).
