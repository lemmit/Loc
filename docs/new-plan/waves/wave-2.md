...
| 2.5 seeder-contract | Opus | `src/generator/_persistence/seed-datasets.ts` (grows into the shared seeder model), `src/generator/dotnet/emit/seed.ts`, `src/generator/elixir/vanilla/seed-emit.ts`, `src/generator/python/emit/seed.ts`, `src/generator/typescript/emit/seed.ts`, `src/generator/java/emit/seed.ts` (exists — was added after the plan was drafted), `src/language/validators/seed.ts` + `src/diagnostics/{messages,unsupported-register}.ts` (validator relaxation the model needed); `test/generator/_persistence/**`, per-backend seed tests | `M-T6.52`: one shared "what does the seeder know" model (persistence kind, ordered create-call params + omission) with five readers and a mutation-proved reader census; event-sourced seeding on elixir (event-append via `create_<agg>/1`) / java / dotnet (create-call args from the model, not `forCreateInput`) now works and is runtime-verified (.NET booted against Postgres). Branch `claude/wave-2-seeder-contract`, 4 commits. Hand-off: `handoffs/wave-2-seeder-contract.md` | **done — ready to fold** |
