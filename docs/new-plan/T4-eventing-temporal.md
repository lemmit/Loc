...
**Phase 1 landed (Hono/node):** grammar (`timerSource` SystemMember + `DURATION` terminal + system-wide `for:` scope), `TimerSourceIR`, cadence validators (`loom.timer-cadence` AST-level; `loom.timer-event-shape`/`-needs-state`/`-source-unbound` IR-level), print arm, obs catalog (`timer_fired`/`timer_skipped_overlap`/`timer_lock_contended`/`timer_emit_failed`), and the Hono `scheduler.ts` (node-cron / setInterval + `pg_try_advisory_lock` single-fire → in-process dispatcher). Owner derived from `migrationsOwner` (no stamp). Byte-identical where unused; generated project tsc-builds. `examples/timers.ddd`.
**Remaining:** .NET (stacked fast-follow on the same IR); Phase 2 (Phoenix/Oban, Python/APScheduler, Java/Spring + timezone `in:` + `overlap: allow`); Phase 3 (durability: `missed: run`, `retry:`, per-env cadence, `ddd verify` join).
