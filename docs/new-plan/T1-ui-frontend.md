...
**Remaining slices:** (5) **client-side pagination** (page/pageSize state → `.slice` + a pager control; needs the typed/int generalization of `ui-factories.ts` `stateBlock`, today string-only); (6) **scaffold wiring** — declare `sortKey`/`sortDir`/`page` state + thread the args in `scaffoldList` (`src/macros/stdlib/scaffold/_body-builders.ts`) so scaffold list pages sort out of the box (fixture re-baseline across frontends; verify Feliz/HEEx still compile with the extra declared state); (7) **client-side filter** (or fold into the existing find-filter bar); (8) Feliz/HEEx sort (Elmish + LiveView) if de-pinned; (9) server-driven slice 2 (`QueryView` unwraps `paged`).
