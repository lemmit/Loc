...
**Remaining slices:** (6) **client-side pagination** (page/pageSize state → `.slice` + a pager control; needs the typed/int generalization of `ui-factories.ts` `stateBlock`, today string-only); (7) **client-side filter** (or fold into the existing find-filter bar); (8) Feliz/HEEx sort (Elmish + LiveView) if de-pinned; (9) server-driven slice 2 (`QueryView` unwraps `paged`).
