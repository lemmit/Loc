...
**Slice 1 landed (PR #1890):** client-side column **sort** on **React** — the optional `WalkerTarget` seams `renderSortableHeader` + `renderSortedRows` (`src/generator/_walker/target.ts`), implemented on `tsxTarget`; `Table(sortKey:, sortDir:, Column(..., sortable:, field:))` in `src/generator/_walker/primitives/table.ts`, gated so every un-ported target stays byte-identical. Test: `test/generator/react/table-sort.test.ts`.
**Remaining slices:** (2) port `renderSortableHeader`/`renderSortedRows` to Vue/Svelte (inline needs a **hoisted computed** — Vue/Svelte markup expressions reject the `as` casts React inlines) + Angular (needs a **component-class method** — no arrow/spread in templates) + Feliz (Elmish `List.sortBy` or fail-fast); (3) **client-side pagination** (page/pageSize state → `.slice` + a pager control; needs the typed/int state generalization of `ui-factories.ts` `stateBlock`, today string-only); (4) **scaffold wiring** — declare `sortKey`/`sortDir`/`page` state + thread the args in `scaffoldList` (`src/macros/stdlib/scaffold/_body-builders.ts`), landed AFTER all frontends consume sort so no target emits unused state; (5) **client-side filter** (or fold into the existing find-filter bar); (6) HEEx pin + Feliz decision; (7) server-driven slice 2.
