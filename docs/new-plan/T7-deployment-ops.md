...
## M-T7.1 — Metrics + OpenTelemetry — `built` · **L** · P1
...
Landed: **Prometheus `GET /metrics` on all five backends** (Hono/Python/.NET/Java/Phoenix). Platform-neutral metric catalog (`src/generator/_obs/metrics.ts`, the log-catalog pattern applied to metrics) → `http_requests_total` + `http_request_duration_seconds` (RED, route-template labels) + each client library's default runtime series, recorded at each backend's request_end seam; every obs-e2e suite scrapes it. Per-backend client: prom-client (Hono), prometheus_client (Python), prometheus-net (.NET), Actuator+Micrometer (Java), telemetry_metrics_prometheus_core (Phoenix). Per-operation + domain-fault counters on all five backends (#2150), and compose/k8s Prometheus **collector wiring** (#2129). **OTel tracing spans on all five backends** (the last piece): a SERVER span per request carries the execution-context ids as attributes (`loom.correlation_id`/`scope_id`/`actor_id`) + the HTTP semantic attrs, and its `trace_id`/`span_id` ride back onto every request-scoped log line (log↔trace correlation) via the same carrier that stamps `scope_id`. Neutral descriptor `src/generator/_obs/tracing.ts`; created on every request, exported via OTLP/HTTP only when `OTEL_EXPORTER_OTLP_ENDPOINT` is set; a bundled **Jaeger** collector wired into compose (UI :16686) + k8s env. All five obs-e2e gates assert the `trace_id` bracket. Per-backend SDK: `@opentelemetry/*` (Hono), OpenTelemetry.Instrumentation.AspNetCore (.NET), io.opentelemetry BOM (Java), opentelemetry-sdk (Python), opentelemetry_api/exporter (Phoenix).
