...
| **2** | Remove orphaned `resolvePersistence()` (§2.5/§2.7-Q1) | — | ✅ **DONE** — closes M-T6.10 core; 6 test files rewired to the live menu; 184 tests green. (Four dead `emit*` methods deferred — entangled with the pinned interface + stub mechanism; see §2.7-Q1.) | `npm test` ✓ |
...
1. **Adapter-emit removal — DONE (partially; the rest is more entangled than §2.5 assumed).** ✅ **`resolvePersistence()` removed** (the precise M-T6.10 orphan — 0 production callers; the 6 test files that used it as an accessor were rewired to the live `adaptersFor(...).persistence[name]` menu; 184 adapter+platform tests green). **Correction to §2.5:** the emit-layer is **not** entirely dead — `PersistenceAdapter.emitProjectDeps` **has a live caller** (`hono/v4/emit.ts:808`), so the interface stays. The confirmed-dead-but-**deferred** set is the four heavy emit methods (`emitConnectionSetup`/`emitRepository`/`emitMigrations`/`emitOutbox`, 0 production callers): removing them is a larger, entangled change (the `stubAdapter` proxy mechanism + `contract-shape`/`stub-throws` tests assert on them, and it touches the D-ADAPTER-HOME-pinned interface + 6 backend adapter impls), so it is left as a **separate focused M-T6.10 follow-up** rather than rushed here. M-T6.10's core orphan (`resolvePersistence`) is closed.
