...
| **Worktree isolation has its own cost: a worktree's `npm install` does not reproduce the workspace symlinks** | Observed in Wave 1 (#2700): the four `test/platform/packaging-split-*` tests walk `node_modules/@loom/*` and fail in a fresh worktree purely because the install never made those links — they pass once the links exist. This is an environment artifact, not a regression, but an agent that does not recognise it will either chase a phantom failure or, worse, "fix" a gate to make it pass. Dispatch note for every worktree agent: **`packaging-split-*` failing in a worktree is expected**; confirm by creating the symlinks before treating it as real |
