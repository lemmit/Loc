...
> **[2026-07-14 refresh — code-verified against `main`]** Most of this list has since shipped; what remains genuinely open is called out per row. Fully done: **row 2** (the correctness set — M-T6.1/6.4/6.7/6.8 all `done`), M-T2.2, M-T9.1, M-T9.2 (concluded), and — not listed here but the same class — **M-T5.6** (strict decimal/money bounds). Partly landed: M-T2.1 (column-rename shipped; **table-rename slice in flight**), M-T9.3 (all five per-PR behavioral gates exist; unit-tier + coverage phases remain), M-T1.1 (**in flight**). Still genuinely open + unclaimed P1: **M-T3.1** (M-T3.4 now `done` — its idempotency slice (c) descoped 2026-07-18). Verify against fresh `main` + open PRs before claiming any of these — the list rots fast.
...
3. **M-T3.1 + M-T3.4** deny-by-default + versioned-on/409-mapper — the security-default flips. *(M-T3.4 `done` — versioned-on #1933 + 409-mapper shipped, idempotency slice (c) descoped 2026-07-18; M-T3.1 still open.)*
