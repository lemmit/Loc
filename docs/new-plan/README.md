...
*Correction (**2026-07-29**): the 07-28 note above briefly listed `write(...)`/`readonly when` write-side field gating as current — it **shipped (#2254) then was reverted the same window** (revert `4d9a7ad`, owner decision: redundant with a row-aware `requires` on the operation). The M-T3.2 mission body in [T3](T3-security-governance.md) is correct ("WON'T DO — reverted"); only read masking (`mask unless`) ships. Other 07-29 deltas verified: M-T3.13 is **in-flight** (open PR #2259), not `open`; M-T9.12 carries a duplicate ID (Schemathesis-`open` in [T9](T9-toolchain-health.md) vs event-sourcing-`done` in [testing-quality-improvement-plan.md](testing-quality-improvement-plan.md)) — the event-sourcing one shipped; the Schemathesis mission needs a fresh number.*
