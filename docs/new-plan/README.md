...
|---|---|---|---|---|
| [T1 — UI & frontend ceiling](T1-ui-frontend.md) | Data-heavy tables, upload, forms tail, state/async, i18n, a11y, extern parity, navigation, the numeric/money frontend seams | 22 | 5 | **P1 — highest product ROI** |
...
| [T4 — Eventing, workflow & temporal](T4-eventing-temporal.md) | Timers/jobs, projections, channels/brokers, outbox completion, saga hardening, realtime contract, email/storage batteries | 9 | 3 | P2 (temporal hole is the biggest single gap) |
| [T5 — Language core & type system](T5-language-core.md) | Exception-less A4–A6, criterion/retrieval tails, payload P3/P5, stdlib tail, inheritance I4, lifecycle 3–5, surface hygiene, numeric RS-rulings | 19 | 6 | P2 |
...
Live = every heading that is not closed (`open` 49 · `partial` 63 · `in-flight` 4 · `blocked` 6 · `plan`/`deferred`/`recurring` 4 · `frozen` 7 — 133 in all, 2026-09-02 count; regenerate with `grep -c '^## M-T' docs/new-plan/T*.md`).
