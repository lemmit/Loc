...
The generated-code validation pyramid is: string-assert → compile → **behaviorally run** → full-stack round-trip. Per-PR, the top two tiers are uneven. Originally **9 of 33 corpus features carried no behavioral block at all** (`test/fixtures/corpus/*.ddd` with no `test e2e`/`test`), proven to *compile* on 5 backends and to *run* on zero. M-T9.12 closed `event-sourcing` + `eventsourced-workflow`, and the list has since become a machine-checked register — `E2E_LESS_CORPUS_FIXTURES` (see M-T9.13 below for the live drained/remaining split; this paragraph's original count is kept as the historical baseline). (Several have *dedicated* runtime e2e legs — `outbox`/`channels-broker` via `channels-e2e.yml`, `tenancy-hierarchy` via `tenancy-e2e.yml` — but those are nightly/post-merge, not per-PR, and the rest have none.)
