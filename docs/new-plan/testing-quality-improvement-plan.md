...
### M-T9.13 — Drain `E2E_LESS_CORPUS_FIXTURES`: runtime callers for the caller-less features — `partial` (re-scoped 2026-08-10) · **M** · P2
**#2468 is this mission's first drain slice** (11 → 8): `projection-aggregation`, `projection-groupby`, `field-mask` gained `test e2e` blocks — chosen as the ones needing no sidecar and no second deployable, all green on node/PGlite with no new uncalled pins — and found **ten bugs** (five in the e2e authoring surface itself: money literal unwritable in a request body, a numeric conversion rendering the domain idiom `.toNumber()` on a wire string, an inline api call as receiver losing its parens, `contains` on a wire string, a free call emitted verbatim → new `loom.e2e-unresolved-call`; one product bug — the field-mask absent-array-claim 500, see M-T3.2; plus the dotnet claim-default and the two dapper runtime skips). The three drained cases still owe **goldens** (deferred deliberately — money/timestamp forms differ across backends and a golden is a reviewed answer key). **Remaining: the 8 register entries.** `policy-deny` first — `deny` compiles on all five backends and "a denied read 404s / lists empty over HTTP" has never been proven at runtime. `resources`/`extern`/`extern-handlers` need a put→get / handler-invoked harness (`extern` bodies that legitimately `throw "not implemented"` stay honest fail-fast, excluded); `api-call`, `channels-broker`, `outbox`, `tenancy-hierarchy` need their nightly legs' assertions re-expressed as per-PR-reachable blocks or an explicit sidecar decision. Also owns the harness fixes that unlock **12 of the 13 census pins**: run `runSeeds` on the node oracle leg (2 pins — and `seed` datasets get their first oracle-backend runtime coverage), a registry-id principal for the tenantRegistry self-scope (10 pins).
