...
The generated-code validation pyramid is: string-assert → compile → **behaviorally run** → full-stack round-trip. Per-PR, the top two tiers are uneven. **9 of 33 corpus features carry no behavioral block at all** (`test/fixtures/corpus/*.ddd` with no `test e2e`/`test`), so they are proven to *compile* on 5 backends and to *run* on zero: `channels-broker`, `event-sourcing`, `eventsourced-workflow`, `extern`, `extern-handlers`, `field-auth`, `outbox`, `resources`, `tenancy-hierarchy`. (Several have *dedicated* runtime e2e legs — `outbox`/`channels-broker` via `channels-e2e.yml`, `tenancy-hierarchy` via `tenancy-e2e.yml` — but those are nightly/post-merge, not per-PR, and the rest have none.)

### M-T9.12 — Event-sourcing runtime validation — `open` · **M** · P1
`event-sourcing` (append-only stream + appliers) and `eventsourced-workflow` (a saga folding its own emitted events) emit on all five backends but are validated **nowhere at runtime** — a wrong fold/replay produces green CI. Add a `test e2e` block to both corpus fixtures (append → read-back the folded state) so the five per-PR `behavioral-e2e-*` legs execute it; assert the replayed aggregate state, not just HTTP 200. Cross-ref: the node ES-create-invariant bug M-T9.3 already surfaced shows this class is live.
