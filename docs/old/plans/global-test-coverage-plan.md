...
- **New `feature-doc-coverage.test.ts`**: cross-reference `docs/<feature>.md` (the 14 feature reference docs) against the manifest — a documented feature with no corpus fixture fails. Keeps docs and tests honest with each other. **Landed** — a curated `FEATURE_DOCS` set (the 14: 13 doc-cited corpus features + `scaffold-macros` as the one acknowledged, ratcheted `KNOWN_GAP`); the gate asserts every documented feature is either corpus-covered or an acknowledged gap, so a new feature doc with no fixture — or dropping a covered feature's last fixture — fails per-PR. Draining a gap = add the `<feature>.ddd` + manifest row, delete the `KNOWN_GAPS` entry (M-T9.3 unit-tier drain). **First drain:** `domain-services.ddd` (pure/reading/mutating tiers orchestrated by workflows) took the domain-services gap from 2→1 — and surfaced a real Python emitter bug (`x == null` emitted `== None`, tripping ruff E711; now `is None`, parity with the union-find `absenceCheck`).
