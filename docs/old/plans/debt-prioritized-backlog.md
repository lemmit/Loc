...
| 15 | 🟢 **DONE (all 4 relational backends)** | part-in-part containments FK to their DIRECT parent (`directParentName`/`directParentOf`, shared with migrations-builder) + persist/read on java (Phase 1), python (Phase 2), node (Phase 3), .NET (Phase 4) — each boot-verified on Postgres (create → nested label → GET-nests → re-save-preserves). Gates removed; single-level output byte-identical. Only construction-bearing NESTED ops (`new Label` inside `new Shipment`) remain a follow-up |
...
| DEBT-15 | ~~Part-declared single (non-collection) containments~~ **DONE — all four relational backends** (java #1596, python Phase 2, node Phase 3, .NET Phase 4): part-in-part nesting FKs to the direct parent + persists/reads on every backend | ~~java / python / node / dotnet~~ | 2 | M | `nested-parts-alignment.md` |
...
- **DEBT-15 Java nested part-containments — DONE (java).** A nested part (`Shipment contains label: Label`, single or collection) now FKs to its **direct parent** (`labels.shipment_id`), not the aggregate root — via `directParentOf` (`src/ir/util/containment-parent.ts`), shared by `migrations-builder.ts` and the java emitter so the JPA join column and the Flyway DDL agree. Boot-verified end-to-end on Postgres (Flyway migrate → SQL-insert a two-level graph → GET nests `label`+`stickers` under the right `shipment`). The gate (`loom.java-single-containment-unsupported`) is gone. **This is Phase 1 of [`nested-parts-alignment.md`](nested-parts-alignment.md)** and the DEBT-15 **deliverable**. Phase-2 scoping then found the cross-backend "alignment" is far larger than the plan implied: node never saves single containments and never loads nested parts; python hard-codes the nested level empty (`label=None, stickers=[]`, with `_hydrate_*` helpers as dead code). So node/python/.NET part-containment was **substantially unimplemented**, not merely mis-FK'd — building it out is a real per-backend feature. **python (Phase 2) + node (Phase 3) landed**: nested parts FK to (and brand `parentId`/`parent_id` from) their direct parent, `save` recurses (diff-sync per nested level; node also now persists single containments, which it dropped entirely before), the read paths recursively load + assemble the nested level, DTOs emit children-first (no forward-ref), both boot-verified on real Postgres. **.NET landed too** (Phase 4): the EF owned-type config nests a part's own containments inside its parent's `OwnsMany` with the shadow FK named for the direct parent; EF materialises/persists the owned graph (no repo changes); `dotnet build /warnaserror` + boot round-trip verified. **All four relational backends now persist + read part-in-part nesting; DEBT-15 is fully drained.** Re-scoped details in `nested-parts-alignment.md`.
...
When we pick one up, spin its row into a focused slice plan under `docs/old/plans/`
