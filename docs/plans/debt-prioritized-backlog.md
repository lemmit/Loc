...
| 02 | 🟡 OPEN (partial) | node (both shapes) + java (both shapes) + **elixir (embedded — its only non-rel shape)** ship; only **principal-on-non-relational** still gated |
...
| DEBT-02 | Non-relational (`shape(document/embedded)`) capability `filter` — **node (both shapes) + java (both shapes) + elixir (embedded) landed** (document → in-app over the rehydrated aggregate; embedded → root scalars are real columns, so SQL `where` / `@SQLRestriction` / Ash `base_filter`). `document` is not an elixir saving shape, so embedded is its only non-rel case. Follow-up: **principal-on-non-relational** (the actor + in-app/json predicate) | ~~node, java, elixir~~ · principal-non-rel | 4 | M | — |
...
- **Where:** same gate as DEBT-01 (`supportsNonRelationalFilter` in `system-checks.ts`).
- **node (landed):** `document` filters in-app over the rehydrated aggregate (`documentCapabilityBody`, `repository-document-builder.ts`); `embedded` root scalars are real columns, so the predicate AND-s into the SQL read like the relational path (`repository-embedded-builder.ts`).
- **java (landed):** `document` filters in-app via `findAll().stream()` (`emit/document-store.ts`); `embedded` rides Hibernate's `@SQLRestriction` like the relational path.
- **elixir embedded (landed):** the embedded root is a real Ash resource / Ecto schema — root scalars are columns, so the predicate lands on every read via the **shape-agnostic** `renderBaseFilter` (Ash `base_filter expr(...)`) / `vanillaCapabilityFilter` (`from(record in <Agg>, where: ...)`); the only change was removing the validator gate (no generator change). Ash fixture (`embedded-filter.ddd`, entity `contains` → `data_layer: :embedded`) compiles against real Ash 3.x — verified locally; vanilla fixture (`vanilla-embedded-filter.ddd`) is scalar-root + filter. `document` is not an elixir saving shape (`PLATFORM_SAVING_SHAPES`), so embedded is its only non-rel case — the elixir **filter** leg of DEBT-02 is complete.
  - *Adjacent gap discovered (NOT a filter issue):* a vanilla embedded aggregate with an **entity `contains`** silently drops the containment — the vanilla schema emitter folds value objects to `:map` but emits no field/embed for entity parts, so the generated `change_<op>` casts the part's fields onto a struct that lacks them (compiles, runtime-broken). Ash handles entity containments correctly (embedded resources). Either gate vanilla entity-in-embedded or emit an `embeds_many`/`:map` for it — separate follow-up.
- **Remaining — principal-on-non-relational:** a *principal* (tenancy) filter on a non-relational shape is still gated everywhere. For **embedded** the root scalar is a column, so it should ride the existing relational principal path (a near-gate-only follow-up on node/java/elixir); for **document** the actor must be injected into the in-app predicate (`requireCurrentUser()` / `CurrentUserAccessor` in `documentCapabilityBody` / the java stream filter) — real generator work.
