...
| DEBT-01 | ~~Principal-referencing capability `filter` (`currentUser` / tenancy)~~ **DONE** — all five backends (node, .NET, elixir Ash + vanilla, java) wire it | ~~node, elixir, java~~ | 5 | L | `proposals/criterion-everywhere.md` · **landed on every backend** (java reified-criterion retrieval the only residual gap) |
