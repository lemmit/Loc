...
| DEBT-02 | Non-relational (`shape(document/embedded)`) capability `filter` — **node landed (both shapes)**: `document` filters in-app over the rehydrated doc; `embedded` AND-s the predicate into the SQL `where` (root scalars are real columns). Follow-ups: elixir, java | ~~node~~ · elixir, java | 4 | M | — |
