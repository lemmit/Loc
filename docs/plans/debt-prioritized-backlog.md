...
| DEBT-01 | Principal-referencing capability `filter` (`currentUser` / tenancy) | ~~node, elixir~~, java | 5 | L | `proposals/criterion-everywhere.md` · **node + elixir (Ash + vanilla) landed** (java the last gap) |
...
- **Today (after node + elixir-Ash + elixir-vanilla slices):** **.NET** (EF `HasQueryFilter`), **node** (Hono/Drizzle), **elixir/Ash** (`base_filter` + `^actor`) and **elixir/vanilla** (Ecto `where` with `^(current_user && current_user.f)` + threaded `current_user`) all wire `filter this.tenantId == currentUser.tenantId`. Only **java** still rejects it.
...
- **Follow-ups (still gated):** **java** — `@SQLRestriction` is static SQL, so principal filters need a Hibernate `@Filter` (session-enabled with params) or query-level AND.
