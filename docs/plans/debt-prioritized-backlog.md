...
| 15 | 🔴 OPEN (gate is CORRECT) | java part-declared single containments — **not a quick win** (investigated): shared `MigrationsIR` forces child-owns-FK, JPA has no unidirectional 1:1 with FK on the child, and parts have no operations so population is under-specified on *every* backend. See detail. |
...
- **DEBT-15 Java single part-containments** — **investigated 2026-06-20: the gate is CORRECT, not a quick win.** Three compounding constraints make a part-declared `contains x: P` (non-collection) genuinely unmappable on Java today: (1) **`MigrationsIR` is shared across every DB backend**, so Java is locked to the same *child-owns-FK* schema node/python use (`labels.shipment_id`) — it can't diverge to a parent-owns-FK shape. (2) **JPA has no unidirectional 1:1 with the FK on the child table** (see `jpa-annotations.ts:74-76`), which is exactly why root single containments use the instance-requiring bidirectional `_parent` `@OneToOne(mappedBy)` — and that needs the parent *instance* at construction. (3) **Parts have no operations**, so a nested single containment can only be populated via a nested constructor `Shipment { label: Label{…} }`, where the parent (Shipment) doesn't exist yet → no instance (and no parent *id*) to thread. The other backends construct the child parent-agnostically (parent **id** value / EF `OwnsOne`), but no working nested-*population* example exists on *any* backend — they simply don't gate the *declaration*. So this is under-specified end-to-end, not a port. Reopening needs a population-semantics decision first (how is a part-declared single containment created?), then likely a Java-specific schema/repository path. `system-checks.ts` `loom.java-single-containment-unsupported`.
