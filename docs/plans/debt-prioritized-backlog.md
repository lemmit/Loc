...
- **`List` / `Detail` / `MasterDetail` — reframed, see proposal.** These overlap the `scaffoldList`/`scaffoldDetails` expander, and the deeper issue is that page-body scaffolding expands as opaque IR-phase-⑤c magic (no printer arm, no unfold) rather than unfoldable source. The direction is **not** a per-frontend renderer but scaffold emitting **real, named components that pages reference**, produced at the macro layer (printable/unfoldable), with `List`/`Detail` becoming thin macros over them. Design-first epic captured in [`../proposals/unfoldable-page-scaffolding.md`](../proposals/unfoldable-page-scaffolding.md) (UI-side twin of `unfoldable-api-derivation.md`).
