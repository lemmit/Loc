...
**Phase 5 — Reified criteria (`Criterion<T>` standalone, `Specification<T>` for queries) (architectural; the higher-priority thread).** Today criteria are *inlined* — a `criterion` body is substituted into each use-site's `ExprIR`, and thus into every backend's LINQ / SQL / Drizzle / Ash predicate. `CriterionIR` exists in the IR but **no backend consumes it**. The reified direction (`docs/proposals/reified-criteria.md`, sibling `docs/proposals/java-backend.md`) reverses that: backends consume `CriterionIR` directly and emit a **constructed predicate object** — the Evans / Spring-Data `Specification<T>` pattern made real in generated code instead of dissolved at compile time.
  ...
  - **The naming rule: render each node to the framework type at the *same layer*; use a Loom-owned name only where the matching framework name sits at the *wrong* layer (or is absent).** This is the principled form of "avoid the naming quirk" — *not* "never touch framework types." Because `Specification` / `Criteria` are overloaded and *backwards* across frameworks (Ardalis `Specification<T>` = the whole query / **retrieval**; Spring `Specification<T>` = predicate-only / **criterion**; JPA `Criteria*` = a query-*builder*, not a predicate — see `experience_gathered.md` → "The 'Specification' / 'Criteria' naming quirk"), the per-backend mapping is:
    ...
    - **.NET retrieval → Loom `Retrieval<T>`**, applied to EF's `IQueryable` directly (lean — `query.Where(crit.ToExpression()).OrderBy(…).Skip(…).Take(…)`, no extra dependency) **or** optionally *rendered as* an Ardalis `Specification<T>` (a swappable realization detail, like the EF-vs-Dapper axis — keep Ardalis internal to the emitter, public type stays `Retrieval<T>`). **Lean is the recommended default** — Loom owns the shape end-to-end, criterion composition is plain `Expression.And/Or`, and it's uniform with Dapper (no Ardalis equivalent there).
    The invariant: `CriterionIR` / `RetrievalIR` are the neutral concepts; the *rendered* type is whatever sits at that layer in each backend's idiom — Spring `Specification<T>` for a Java criterion, a Loom `Criterion<T>` for a .NET one — and the reader is never handed a framework name at the wrong layer.
  ...
  - **Sequencing — evaluate-face-first.** `IsSatisfiedBy` (invariants / preconditions) needs no expression-tree translation, so it's the simplest first slice and touches no query code; add `ToExpression()` for the query positions (find / view / capability filter) next; bundle into a `Retrieval<T>` (the generic `repo.run(retrieval)`) last. Additive per backend (new emitted types — no flag day), so one backend can adopt it while others keep the inline output. On **.NET** this turns the current **EF Core + inline LINQ** into **EF Core + Loom `Criterion<T>` / `Retrieval<T>`** — the `Retrieval<T>` applied to `IQueryable` directly (lean default) or via Ardalis `Specification<T>` (opt-in). On **Java** the same `CriterionIR` renders to a Spring `Specification<T>` (the criterion) + a Loom `Retrieval<T>` (the bundle).
