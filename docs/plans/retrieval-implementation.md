...
6. **PR3-D-1 — Phoenix retrieval read action** (Ash `read :<name>` with `^arg` filter + `prepare build(sort:)` + offset `pagination`, `run_<name>_<agg>` code-interface define) + `phoenix-build` gate. ✅ **DONE** (this branch). Adds a `filterArgs` render mode (`param` → `^arg(:name)`) + per-type Ash arg mapping. `for-each` stays gated — the loop is PR3-D-2.
7. **PR3-D-2 — Phoenix `Enum.reduce_while` workflow loop** (the `with`-chain → sequenced-body reshape; `repo-run` bind via the `!` bang variant; loop body op-calls in `{:cont}/{:halt}` form). Iterates `matched.results` (offset pagination returns `%Ash.Page.Offset{}`, not a bare list).
