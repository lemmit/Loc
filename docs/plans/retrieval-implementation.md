...
8. **PR4 — explicit `loads` / LoadPlanIR fetch realisation + loads-sufficiency** (the actual eager-fetch wiring; backends honoured `whole` as a no-op until here).
   - **PR4-Phoenix — Ash `load` realisation.** ✅ **DONE** (this branch). The retrieval read action emits `prepare build(load: [:rel, …])` from the `loadPlan`: `whole(T)` loads **every owned containment relationship** (`has_many`/`has_one`) so a downstream `for`-loop operation can read `record.<part>` without a `%NotLoaded{}` crash; an explicit `loads:` narrows the set to the containments its paths root at. Cross-aggregate refs (`X id` → `many_to_many`) stay ids and are never loaded; `embedded`/`document` aggregates fold parts inline and emit no `load`. Ash realises `load` as a separate batched query per relationship, so it composes with the action's offset pagination without the in-memory collection-paging penalty an ORM join-fetch hits (`renderRetrievalAction` / `retrievalLoadAtoms` in `repository-emit.ts`). roster.ddd gains a `contains badges` containment so `ByName`'s whole-load compiles `prepare build(load: [:badges])` under the `mix compile` gate. **`loads`-sufficiency (`loom.retrieval-loads-insufficient`) and the Drizzle/EF slices remain open.**
   - **PR4-EF — owned-types no-op (documented).** On .NET, EF Core owned types (`OwnsMany`/`OwnsOne`, the embedded-shape mapping) are materialised **inline with the root by definition** — there is no lazy/`Include` step to gate, so a `loads`/`whole` plan over owned containments is a deliberate no-op there. Relational (non-owned) child loading is the EF slice's concern when it lands; tracked, not yet wired.
