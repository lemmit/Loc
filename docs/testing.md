...
| **Generated frontend runtime** | The built React/Vue/Svelte/Angular bundle actually **runs** — `vite preview` (`ng build` + a static SPA server on Angular) + the emitted Playwright smoke spec (every param-less route loads). Pure client-side, no backend. | `npm run test:react-e2e` / `:vue-e2e` / `:svelte-e2e` / `:angular-e2e` | `generated-react-e2e` / `generated-vue-e2e` / `generated-svelte-e2e` / `generated-angular-e2e` | ❌ **post-merge** (`push: main`) / dispatch |
| **Observability e2e** | The generated backend emits the catalog envelope on stdout (per backend). | `npm run test:obs` (+ `:obs-dotnet/-phoenix/-java/-python`) | `*-obs-e2e.yml` (×5) | ❌ **post-merge** (`push: main`) / `run-obs` label / merge queue |
...
*Per-PR column verified against the workflow `on:` blocks 2026-09-03. **A promotion is in flight** (wave G1 of the 2026-09 verification-waves plan under `docs/new-plan/`) that adds path-scoped `pull_request` triggers to the five `*-obs-e2e` workflows and the four SPA smokes; when it lands, those two rows become per-PR and this note goes with them.*
