...
| `pairwise.yml` | `npm run test:pairwise-corpus` (per-PR leg) · `test:pairwise-corpus-tsc` · `test:pairwise-corpus-schema-load` (or `LOOM_MIGRATION_PG_URL`); shard the compile leg with `LOOM_PAIRWISE_TSC_CASE=<case-id>`, dump the composed sources with `LOOM_PAIRWISE_DUMP=<dir>`, get the verdict census with `LOOM_PAIRWISE_REPORT=<file>` | docker (schema-load leg) |
