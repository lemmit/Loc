...
| **Per-backend build** | Generated backend **compiles** clean. TS (`tsc --noEmit` + tsup), .NET (`build /warnaserror`), Java (`gradle bootJar`), Python (`uv` + ruff + mypy + pytest), Elixir (Ash + vanilla `mix compile --warnings-as-errors`). | `npm run test:tsc` / `:dotnet` / `:java` / `:python` / `:phoenix` | `hono-build` / `dotnet-build` / `java-build` / `python-build` / `elixir-{ash,vanilla}-build` | ✅ |
