...
| `channels-e2e.yml` | `npm run test:channels` family — `test:channels{,-mikroorm,-python,-dotnet,-java,-elixir}`, `-rabbit*`, `-kafka*`, `-auth` (pg via `LOOM_CHANNELS_PG_URL`; redis/rabbit/kafka via docker; `-mikroorm` is the redis leg with `LOOM_CHANNELS_PERSISTENCE=mikroorm`, shifted ports so it runs beside the default one) | docker, pg |
