...
| ▢ | **Anonymous `filter` capability** predicates reify (the #760/#762 `contextFilters` mechanism) — **Hono shipped** (a `filter <Criterion>` calls the module-level `<name>Criterion` fn, deduped with find/retrieval consumers; `AggregateIR.contextFilterRefs` carries the ref); Phoenix/Ash remains | ~~Hono~~, Phoenix/Ash | 1, 3 | — |
