...
| ◐ | **Principal/tenancy** (`currentUser.<field>`) bound at the reified query-face. **.NET retrieval query-face shipped (DEBT-24):** a `retrieval` whose `where` references `currentUser` builds its predicate inside the `Specification<T>` ctor — a static position — and now resolves the principal through the same ambient accessor the EF `HasQueryFilter` capability filters use (`RequestContext.Current!.CurrentUser!`, the shared `AMBIENT_CURRENT_USER`), so the backend has **one principal source**. This *fixed a latent compile bug*: the inlined spec previously named an unbound `currentUser` (CS0103). Still ▢: reifying a principal criterion into a `Criterion<T>` *object* (it's still excluded from the `Criterion<T>` class and falls to the inline spec path), retiring the `usesUser` find-parameter threading, and the Hono/Phoenix query-faces (the held-#767 case) | .NET retrieval ✓; Criterion-object + `find` threading + Hono/Phoenix ▢ | 2, 3 | (holds #767) |
