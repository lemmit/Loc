...
| ◐ | **Principal/tenancy** (`currentUser.<field>`) bound at the reified query-face. **.NET + Hono find/retrieval query-faces shipped (DEBT-24):** a `find`/`retrieval` whose `where` references `currentUser` reifies its predicate into a static position (.NET `Specification<T>` ctor; Hono module-level `<name>Criterion` fn) and now resolves the principal through the same ambient accessor the capability filters use — .NET `RequestContext.Current!.CurrentUser!` (the shared `AMBIENT_CURRENT_USER`), Hono `requireCurrentUser()`, so each backend has **one principal source**. This *fixed a latent compile bug on both*: the reified predicate previously named an unbound `currentUser` (.NET CS0103 / Hono `tsc` `Cannot find name`). Still ▢: reifying a principal criterion into a `Criterion<T>`/named *object* (it stays excluded from the `Criterion<T>` class and falls to the inline-predicate path), retiring the `usesUser` find-parameter threading (the now-redundant param is left in place), and the Phoenix query-face (the held-#767 case) | .NET + Hono find/retrieval ✓; Criterion-object + `usesUser` retirement + Phoenix ▢ | 2, 3 | (holds #767) |
