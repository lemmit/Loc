...
3. **`docs/proposals/storage-and-platform-config.md`** — flip `Status:` to "Foundation merged; feature implementations pending per phases listed below."

4. **`docs/proposals/storage-and-platform-config-micro-plan.md`** (this file) — add a "What's stubbed" appendix listing every error message and its tracking phase.
