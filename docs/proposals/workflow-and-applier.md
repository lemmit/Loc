...
**Phoenix / Ash event sourcing — deferred (not implemented; design landscape below).** Phoenix is the one remaining ES gap. It is deferred for *effort + validation* reasons, **not** because Ash lacks event support — the Ash ecosystem now ships first-class options, so the gap is "pick the adapter and build it blind against CI," not "fight the framework."
