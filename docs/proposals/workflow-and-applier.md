...
**Phoenix / Ash event sourcing — deferred (recon complete; not implemented).** Phoenix is the one remaining ES gap, deliberately deferred after a recon that rated it ~4/5 difficulty against the cheap second-adapters. The architectural mismatch:

- **Ash assumes a state table.** An `Ash.Resource` with `AshPostgres.DataLayer` maps to a single state row; event sourcing has no state table, only an append-only stream. The whole resource-to-table assumption has to be bypassed for eventLog aggregates.
- **The functional changeset model fights emit-and-append.** On the OO backends, `emit` mutates an aggregate instance in place (push the event onto a list **and** fold it into `this`). Phoenix's domain logic lowers assignment to `Ash.Changeset.change_attribute` and `emit` to a `Phoenix.PubSub.broadcast` — neither has a place to accumulate-and-replay a stream within a single command.
...
Design options the recon surfaced, for whoever picks this up:

1. **Plain Elixir fold module + Ecto events table + thin repository (recommended).** Sidestep Ash's data layer for eventLog aggregates: emit a plain `<Agg>Fold` module (`from_events/2` + per-event `apply/2`), an Ecto-managed `<agg>_events` table (matching the shared `MigrationsIR` shape), and a hand-rolled repository module that reads the stream → folds → appends on save. The aggregate is *not* an `Ash.Resource`; the read/query surface is custom.
2. **Ash manual actions / a custom `Ash.DataLayer`.** Keep the aggregate as an `Ash.Resource` but back it with manual actions (or a bespoke data layer) that read/append the event stream. Far more Ash-idiomatic but substantially more work, and the custom data layer is the long pole.
