...
**Phoenix / Ash event sourcing — deferred (not implemented).** Phoenix is the one remaining ES gap. The decision criterion is **idiomatic Ash code** — a generated Phoenix project must read like one a Phoenix engineer would write. Under that bar, the deferral is *not* "too much effort"; it's that **no idiomatic Ash path can express Loom's `persistedAs(eventLog)` semantics today**, and the non-idiomatic path (a hand-rolled fold module) is ruled out on principle.

Loom's `persistedAs(eventLog)` is *pure, explicit* event sourcing: the modeller **declares** domain events (`event Deposited { amount: int }`), **emits** them deliberately from emit-only command bodies, and writes **appliers** (`apply(e: Deposited) { balance := balance + e.amount }`) whose fold *is* the state logic — with **no state table** (the per-aggregate stream is the sole source of truth, folded on every load). The Hono and .NET backends implement exactly this.

Why the idiomatic Ash options don't fit (verified against their docs, mid-2026):

- **[AshEvents](https://hexdocs.pm/ash_events)** (v0.7) is **semantically incompatible**, not merely a partial fit. It is *event logging on top of state-backed resources*, not domain-event folding: events are **auto-derived from action invocations** (`action`, `data` = inputs, `changed_attributes`) — there is **no way to declare custom event types** or `emit` them; there is **no applier / `apply(state, event)` reducer** the developer writes; **replay re-runs the original actions** chronologically (`replay_overrides … route_to …`), it does not fold events; and the resource **keeps its state table** (the `AshEvents.EventLog` is an audit/replay sidecar) — a *pure* event log with no state table is not expressible. Mapping Loom onto AshEvents would mean discarding `event`/`emit`/`apply` entirely — a different feature, not Loom's event sourcing.
- **[AshCommanded](https://hexdocs.pm/ash_commanded)** (Ash + [Commanded](https://github.com/commanded/commanded)) is the *one* ecosystem piece whose model matches — Commanded aggregates have command handlers that return events and a real `apply/2` fold, with native per-aggregate streams. But it is a **separate ES framework with its own EventStore**, not `AshPostgres`/Ecto-idiomatic, and it diverges from the shared `<agg>_events` `MigrationsIR` table the other backends use. Adopting it is a deliberate, heavyweight product choice — out of scope for now, and not "idiomatic Ash" in the `AshPostgres` sense.

So: until either AshEvents grows explicit-event + applier-fold support, or Loom decides to take on AshCommanded as a first-class Phoenix ES backend, **Phoenix `persistedAs(eventLog)` stays a hard IR-validate error**. (A non-Ash hand-rolled `<Agg>Fold` module + Ecto `<agg>_events` table + thin repository would *compile* and match the other backends, but it is explicitly **not** idiomatic Ash and is therefore not the path.)

The validator gate stays as-is: `EVENT_SOURCING_BACKENDS` in `src/ir/validate/checks/system-checks.ts` excludes `"phoenix"`, `ash-postgres-persistence.ts` advertises `["state"]` only, and `test/ir/eventsourced-storage-support.test.ts` pins the Phoenix-rejects behaviour. Adding `"phoenix"` to that set is the one-line un-gate once an idiomatic backend exists.

**Still deferred** (beyond Phoenix ES above): snapshots, projections / read models, and the workflow-as-aggregate / `on(...)` handler surface. Reframes today's `workflow Name(params) [transactional]` (see [`docs/workflow.md`](../workflow.md)). Companion to the events surface declared today (`event Name { ... }`) and complementary to the schemas-as-boundary commitment that the wire-spec artifact already approximates but currently sources from the wrong layer. Sagas (compensation contract) deferred to a v2 amendment.
