...
| [`render-expr-target-unification.md`](./render-expr-target-unification.md) | PROPOSED (sequenced onto A4) | Unify the three structurally-identical `render-expr.ts` dispatchers (TS 379 / .NET 467 / Phoenix 547 LOC — same 17 `ExprIR.kind` arms, leaf-divergent only) behind an **`ExprTarget`** contract, mirroring the body-walker's `WalkerTarget` extraction (PRs #607–#627). The contract captures the eight real divergence axes (operators, naming, money arithmetic, collection ops, `refColl.contains` membership, regex, `ref` role, `callKind` call syntax); the 17-arm dispatch + recursion go shared. Payoff: a 5th domain-logic backend writes **one target table**, not a 4th hand-rolled dispatcher. **Deliberately not standalone** — pinned to ride the exception-less **A4** PR (which already rewrites `render-expr`/`render-stmt` heavily per `exception-less.md` + §5.1 of the type-system migration plan), so the new `?`-propagation / or-union arms are authored once behind the contract rather than re-threaded. The one A4-independent slice (`refCollectionFieldName` → `ir/util`) already landed (#793). Byte-identical-output gated per seam. |
