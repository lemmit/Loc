...
| [`unfoldable-page-scaffolding.md`](./unfoldable-page-scaffolding.md) | PROPOSED | **UI-side twin of [`unfoldable-api-derivation.md`](./unfoldable-api-derivation.md).** Page-body scaffolding (`scaffoldList`/`scaffoldDetails` sentinels + the `List`/`Detail`/`MasterDetail` archetypes) explodes into an inline tree at IR lowering phase ⑤c — no printer arm, no unfold, the user can't see/edit/eject it. Proposes scaffold emit **real, named `component`s that pages reference**, produced at the **macro layer** so they print and unfold like every other macro; `List`/`Detail` become thin macros over those components (DEBT-05's List/Detail half dissolves — no new walker renderer). Reuses the existing `expandScaffoldList`/`Details` tree-builder, moved from lowering to macro `expand()`. Requests `D-UNFOLDABLE-PAGES`. |
