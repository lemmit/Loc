...
| [`channels.md`](./channels.md) | PROPOSED | **The messaging / transport tier** — fills the "async messaging/outbox" gap in `production-readiness.md`. A context-member `channel { carries / delivery / retention / key }` (many per context, like a `view`) unifies pub/sub, work-queue, and durable stream via orthogonal `delivery`×`retention` knobs; `channelSource` binds it to a `storage{type: redis/kafka/rabbitmq}` (mirrors `persistedAs(eventLog)`/`dataSource`, [D-STORAGE-SPLIT](../decisions.md#d-storage-split--split-the-overloaded-storage-keyword)) — the contract names no transport. Reuses the pinned `on(e: Event)` / `projection` consumer surface (reactor body = workflow body); producer `emit` unchanged (the `DomainEventDispatcher` seam). Also owns **realtime delivery to the browser**: the SSE/WebSocket wire (platform-derived, infra not contract), the two-hop **edge relay** (cross-DU), and **delivery scoping** — *who may receive a pushed event* via `DataKey` rooms + subscribe-time admission. The read-side counterpart (what to refetch / caching) is split into `caching.md`. |
| [`caching.md`](./caching.md) | PROPOSED | **The read-side companion to `channels.md`** — reads, freshness & invalidation; fills `production-readiness.md` §3.4. The cache/invalidation/realtime-routing key is the **React Query key** (interest), *not* `DataKey` (visibility). Because the origin knows exactly *when* data changes (`save` → ticket), caching is **invalidation-based** (cache hard, purge exactly) not expiration-based: surrogate-key / cache-tag purging (Fastly/Cloudflare/Varnish; ASP.NET `RemoveByTagAsync`; Redis reverse-index) keyed by a derived **dependency set** (lists→type tag, joins→union, wide sets→`projection`). One **event→query-keys** map (`EventInvalidationIR`) drives the server cache, the CDN, the realtime relay, and the client `invalidateQueries` alike. `.live` = subscribe to a query key's invalidations → refetch through the authz'd read (so per-user filtering never enters the push layer); broad invalidation is safe (active-only refetch + coalescing + the cache absorbing N→1 DB reads). **The cache *tier* is gated by authz shape**: public/tenant → edge/OutputCache; per-user → in-handler read-through *below the auth gate*. Surface: `cached: none | tagged`. |
