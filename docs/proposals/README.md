...
| [`caching.md`](./caching.md) | PROPOSED | **The read-side companion to `channels.md`** — reads, freshness & invalidation; fills `production-readiness.md` §3.4. The cache/invalidation/realtime-routing key is the **React Query key** (interest), *not* `DataKey` (visibility). Because the origin knows exactly *when* data changes (`save` → ticket), caching is **invalidation-based** (cache hard, purge exactly) not expiration-based: surrogate-key / cache-tag purging (Fastly/Cloudflare/Varnish; ASP.NET `RemoveByTagAsync`; Redis reverse-index) keyed by a derived **dependency set** (lists→type tag, joins→union, wide sets→`projection`). One **save→query-keys** map (`InvalidationRuleIR`) drives the server cache, the CDN, the realtime relay, and the client `invalidateQueries` alike. `.live` = subscribe to a query key's invalidations → refetch through the authz'd read (so per-user filtering never enters the push layer); broad invalidation is safe (active-only refetch + coalescing + the cache absorbing N→1 DB reads). **The cache *tier* is gated by authz shape**: public/tenant → edge/OutputCache; per-user → in-handler read-through *below the auth gate*. Surface: `cached: none | tagged`. |
