...
| [`exception-less.md`](./exception-less.md) | Exception-less flow | `error` payloads (HTTP-blind in the domain); `option` ML-postfix sugar for `T or none`; `?` propagation operator dispatching on `error`-marked variants; api-surface `status <Error> <Code>` mapping + stdlib defaults driving auto-generated RFC 7807 ProblemDetails translation at the api edge; find-variant re-shape; two-regime split (aggregate-invariant throws vs boundary-returns-carrier); no `Result<T, E>` / `Ok` / `Err` wrappers — `T or E` direct |
