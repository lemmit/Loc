...
| [`exception-less.md`](./exception-less.md) | Exception-less flow | `error` payloads (HTTP-blind in the domain); `option` ML-postfix sugar for `T or none`; `?` propagation operator dispatching on `error`-marked variants (lives primarily in workflow bodies — see `docs/workflow.md`); `Repo.getById` re-shape to `T or NotFound`; workflow `precondition` becomes typed `PreconditionFailed` return; api-surface `status <Error> <Code>` mapping + stdlib defaults driving auto-generated RFC 7807 ProblemDetails translation; aggregate-vs-workflow-vs-api layer-aware failure model; no `Result<T, E>` / `Ok` / `Err` wrappers — `T or E` direct |
