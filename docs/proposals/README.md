...
| [`channels.md`](./channels.md) | PROPOSED | **The transport tier for events** — fills the "async messaging/outbox" gap named in `production-readiness.md`. A context-member `channel { carries / delivery / retention / key }` (many per context; owns a subset of *its* context's events, like a `view`) unifies pub/sub, work-queue, and durable stream via orthogonal `delivery`×`retention` knobs. The contract names no transport: `channelSource` binds it to a `storage{type: redis/kafka/rabbitmq}` (the channel(domain)/channelSource(system) split mirrors `persistedAs(eventLog)`/`dataSource` per [D-STORAGE-SPLIT](../decisions.md#d-storage-split--split-the-overloaded-storage-keyword)), and browser realtime is derived from a UI `.live` subscription with the SSE-vs-WebSocket wire chosen by platform (overridable on the *deployable*, never the channel). Per-tenant / per-user fan-out is handled by **subchannels (rooms)** whose key splits **authorization** (the subscriber's `DataKey` prefix from `authorization.md` / `tenancy by` — JWT-pinned, unforgeable; *reused, not redefined*) from **interest** (the channel `key:` leaf the page's `.live` binding selects — `byId(x)`→one, `all`→wildcard), so a detail page gets one aggregate, not the customer firehose, and a browser only receives what its claims permit. That room key is deliberately the **same** `DataKeyPathIR` the future caching proposal (`production-readiness.md` §3.4) will evict by and the React Query client key already mirrors — one canonical address, three consumers. Reuses the pinned `on(e: Event)` / `projection` consumer surface from `bounded-context-model.md` (reactor body = workflow body). Producer `emit` unchanged — the existing `DomainEventDispatcher` seam is the implementation point. |
