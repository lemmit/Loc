...
| [`nextjs-frontend.md`](./nextjs-frontend.md) | PROPOSED (**deferred** — two framings; Path A is the concrete one) | **Two coherent framings, decided by who owns the domain.** **(A) Next as a frontend paired with / hosted by a domain-owning node backend (Nest/Hono)** — the recommended, cheap path: it **rehabilitates the "React shell" reading** (an anti-pattern *standalone*, but *correct* with a backend present, because the domain lives in the backend and Next's server tier becomes a real **BFF/SSR** layer). A node backend can even **host Next's SSR in one process** (`nest hosts: next`) — the node twin of phoenix-embeds-React, with SSR-hosting capability *derived* from being a node runtime (dotnet/elixir can only embed Next's static export). **(B) Next as a standalone full-stack `node` foundation** — one deployable owning TS domain + React UI, the topological analogue of Phoenix+LiveView; heavier, re-introduces two-axis-freezing, forfeits the wire contract → deferred. Inviolable rule across both: **exactly one domain owner**; Next never reaches into domain logic. Both gated behind `embedded-frontend-composition.md` + the `foundation:` axis work. |
