...
| [`unfoldable-page-scaffolding.md`](./unfoldable-page-scaffolding.md) | PARTLY SUPERSEDED | **UI-side twin of [`unfoldable-api-derivation.md`](./unfoldable-api-derivation.md).** Its archetype half is moot: `List`/`Detail`/`MasterDetail` were inert duplicates of `scaffoldList`/`scaffoldDetails` and were **removed** ([D-NO-PAGE-ARCHETYPES](../decisions.md#d-no-page-archetypes)). What survives (OPEN): the phase-⑤c scaffold expansion (`scaffoldList`/`scaffoldDetails` → an inline ~100-line tree) is opaque "magic" with no printer arm / no unfold; the doc's proposal — scaffold emits a **real, named `component`** the page references (inspectable, unfoldable, embeddable) — still applies to the sentinels themselves. |
