...
| [`channels.md`](./channels.md) | PROPOSED | **The transport tier for events** — fills the "async messaging/outbox" gap named in `production-readiness.md`. A context-member `channel { carries / delivery / retention / key / realtime }` (many per context; owns a subset of *its* context's events, like a `view`) unifies pub/sub, work-queue, and durable stream via orthogonal `delivery`×`retention` knobs; `channelSource` binds it to a `storage{type: redis/kafka/rabbitmq}` — the channel(domain)/channelSource(system) split mirrors `persistedAs(eventLog)`/`dataSource` per [D-STORAGE-SPLIT](../decisions.md#d-storage-split--split-the-overloaded-storage-keyword). Reuses the pinned `on(e: Event)` / `projection` consumer surface from `bounded-context-model.md` (reactor body = workflow body) and adds WebSocket/SSE realtime push to the UI (`.live` refs; native on Phoenix LiveView). Producer `emit` unchanged — the existing `DomainEventDispatcher` seam is the implementation point. |
