...
| [`typed-capabilities.md`](./typed-capabilities.md) | PROPOSED | Promotes the stringly-typed capability surface (`implements "X"` / `filter for "X"` / `stamp for "X"`) to a first-class `capability { requires / provides / filter / stamp }` declaration with typed refs + conformance verification. Subsumes the `audit`/`softDelete` field+filter+stamp macros (they collapse into capability decls); `crudish`/`scaffold*` stay macros (structure/operations). Byte-identical-IR migration. Motivated by [`multi-tenancy-design-note.md`](./multi-tenancy-design-note.md) R5's `tenantRegistry requires parent` conformance need. Evolves the *mechanism* of [`../capabilities.md`](../capabilities.md). |
