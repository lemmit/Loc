...
| [`agent-tools-and-mcp.md`](./agent-tools-and-mcp.md) | PROPOSED (spec) | How the toolkit operations become **agent-callable tools**: one transport-neutral **tool catalog** (`src/tools/`) over `src/api/`, surfaced by an **MCP stdio server** (`packages/ddd-mcp`, external hosts) and **direct/in-memory dispatch** in the in-browser playground chat. Tools are pure + stateless (model passed in; no fs side effect), so the server is safe by default and the playground reuses the identical catalog. Pinned [D-AGENT-TOOLS](../decisions.md#d-agent-tools--one-tool-catalog-over-the-toolkit-mcp-and-in-browser-are-transports). |
| [`language-services-and-agent-tools.md`](./language-services-and-agent-tools.md) | PROPOSED | Narrow follow-ups on the shipped LSP / editor-adapter work (slices 5–6, `src/api/lsp.ts` + `src/language/fix-hints.ts` + the `loom.bare-aggregate-in-type` quick-fix). Three asks: **(1)** fix the silent rename correctness bug — `Operation` is missing from `isRenameableMember` (`src/language/lsp/member-refs.ts:55`), so renaming an operation declaration rewrites the decl token but leaves every `order.close()` call site dangling; the test suite covers only Property and Aggregate. **(2)** Close the worst coverage holes — rename (no `prepareRename`, no shadowing, no multi-file, cross-ref tested only for Aggregate), references (3 tests), semantic tokens (1 test for 8+ kinds), hover unresolved-ref rendering (`?` fallback). **(3)** Add the next batch of `fix-hints.ts` providers (`loom.reserved-derived-on-vo`, `loom.seed-id-needs-raw`, `loom.es-tph-forced-own-table`, `loom.legacy-{part,vo}-call`, `loom.react-deployable-missing-ui`, `loom.criterion-arity`) so they ride the shipped `fixHintCodeActions` adapter into Monaco + VS Code with one entry per code. Plus one new refactor: `Fold to macro` (inverse of `Unfold macro`, opt-in per macro via a `foldable: true` tag). Agent-tool surface itself is out of scope — covered by [`agent-tools-and-mcp.md`](./agent-tools-and-mcp.md); fix-hints added here ride that catalog for free via `loom_validate`'s report. |
