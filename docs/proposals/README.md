...
| [`nextjs-frontend.md`](./nextjs-frontend.md) | PROPOSED (**parked** — considered, not pursued) | Why a Next.js target is **not** on the roadmap despite being the dominant new-React choice. Next.js isn't a new framework — it's React (which Loom already targets), so it'd render the same body-walker components into a different *project shell*. The catch: Next.js's reason-for-existing (**React Server Components + Server Actions**, full-stack, blurring frontend/backend) directly opposes Loom's strict wire boundary (`wire-spec.json`, the pipeline-layering invariant, "frontend runs no domain logic"). Conservative targeting (client/SSR, wire preserved) bypasses everything Next.js is *for*; idiomatic targeting (server actions → domain) breaks the boundary. Marginal value is low for Loom's CRUD-over-aggregates app shape (SSR/SEO rarely matter there). If ever pursued, scope as a **React project-shell variant** (app-router + optional SSR), not a full-stack target — ranked below Angular and the in-flight Vue/Svelte. |
