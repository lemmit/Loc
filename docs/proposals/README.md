...
| [`nextjs-frontend.md`](./nextjs-frontend.md) | PROPOSED (**deferred** — foundation, gated) | **Reframed** from "parked React shell": the interesting Next.js target is **not** a second React frontend, it's a **full-stack `node` foundation** — one deployable owning TS domain logic *and* its React UI, the **TypeScript analogue of Phoenix+LiveView** (same topological role: domain+UI in one deployable; *not* LiveView's diff-over-socket mechanism). Reuse is the headline — both halves **already exist** (TS `render-expr`/`render-stmt` from Hono + the React body-walker). But it's a `foundation`-level decision that re-introduces the two-axis-freezing complexity `vanilla-phoenix-foundation.md`/`embedded-frontend-composition.md` are untangling, chases a model that isn't the clear TS default, and forfeits the wire contract. **Deferred, gated behind `embedded-frontend-composition.md` + the `foundation:` axis work.** The naive "React project-shell" reading is an anti-pattern; skip it. |
