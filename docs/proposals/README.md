...
| [`failure-taxonomy.md`](./failure-taxonomy.md) | DESIGN NOTE — revisits `exception-less.md` | Step-back rethink of the whole error story. Keeps the structural core (errors-as-data, HTTP-blind domain + edge `httpStatus`, two-regime throw/return, dependency-direction layer visibility, inline opt-in translation); reconsiders the ergonomics (`?` operator, carrier-monad stdlib — likely drop); adds first-class **declarative validation** (auto-422) as the highest-leverage missing piece. Reframes "exception-less" as a five-kind **failure taxonomy** (absence / validation / expected-domain / bug / integration), each getting the lightest mechanism. Settles error placement on three constructs (`operation` / `workflow` / `api`) + a declarative policy bucket. |
| [`domain-service.md`](./domain-service.md) | DESIGN NOTE (options) | The missing **third construct** between `operation` (single-aggregate domain) and `workflow` (application orchestration): a stateless cross-aggregate **domain rule** that can fail with a domain error (the `transfer → InsufficientFunds` case). Fills the row `failure-taxonomy.md` left empty. Defining constraint: **no infrastructure** (domain objects in, no repo/extern/persistence) — enforceable by the existing layering test. Presents three assembled **shapes**: A "pure calculator" (no mutation, callable anywhere — minimal floor), B "coordinator" (mutates passed aggregates, workflow persists — the transfer case), C "unified function family" (layer inferred from dependency footprint — north star, deferred). Recommends A-first, B fast-follow. Options across 6 axes. |
