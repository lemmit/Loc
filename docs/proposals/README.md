...
| [`lifecycle-url-style.md`](./lifecycle-url-style.md) | PINNED ([D-URLSTYLE](../decisions.md#d-urlstyle--lifecycle-url-style-on-the-api-body--per-action-routeslug)) — Phase 1 (kind-tagged `create`/`destroy` + `creates`/`destroys`/`canonical*` IR) shipped #722; `urlStyle:` grammar + `OperationIR.routeSlug` enrichment exist, but **no backend route emitter reads `routeSlug` yet** (the consuming slice is the remaining work) | Reconciles `lifecycle-operations.md` Phase 2 with the **actual** `api` grammar (which differs from that proposal's assumption). `urlStyle: literal \| resource` on the api body + per-action `routeSlug`. |
...
Tier 1 — broken or misleading surface (fix first)
  Union-returning find producer path (.NET NotImplementedException /
  Hono `unknown` fallback); REMOVE the dropped `?` propagation
  operator; React renderers for Switch/MultilineField/SelectField;
  docs honesty (status headers kept in sync with code).
...
Tier 2 — nearly done (per-backend completion)
  ...
  adapters → HEEx primitive backfill. Plus: routeSlug consumption,
