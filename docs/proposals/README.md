...
| [`nestjs-backend.md`](./nestjs-backend.md) | PROPOSED (**candidate — ahead of PHP**) | Structured/enterprise TypeScript backend (NestJS — modules/DI/decorator controllers, TypeORM/MikroORM, optional `@nestjs/cqrs` bus) modelled as **`transport: nest` on `platform: node`**. The decisive fact: it **reuses the existing TS domain emission** — the Hono backend's `TS_TARGET` (`render-expr`) and `render-stmt.ts` render the *same TypeScript* domain logic verbatim, so the new work is only the framework *shell* (controllers/DI/providers/`class-validator` DTOs/bootstrap). **~half a new-language backend** (~3–5 wks parity / ~1.5–2.5 wks skeleton). The **Angular pairing** (shared module/DI/decorator philosophy) and a real DDD story Hono lacks (modules ≈ contexts, `@nestjs/cqrs` = MediatR analog). Adds *depth* (enterprise tier of an ecosystem Loom already serves), not *reach* — behind Go, **ahead of PHP** (cheaper, no new-language renderer). |
