...
| Doc | Status | Core addition |
|---|---|---|
| [`document-and-json-hierarchies.md`](./document-and-json-hierarchies.md) | PROPOSED | Persisting hierarchies as JSON documents (Marten / EF Core `.ToJson()` / Mongo-embedding analogues) instead of normalised tables. Separates open-shape `json` field (need A) from document-mapped typed hierarchy (need B). **Chosen direction:** two orthogonal per-aggregate axes that are *different in kind* — a body-structure marker (bare `eventSourced`, also a storing-as-a-log facet) × a saving axis (new `storeAs(document)`, paren header modifier) — so the required **`eventSourced` + `storeAs(document)`** (stream + document snapshot, Marten's sweet spot) is expressible. Wired via `storeAs: document` on the `snapshot`/`state` `dataSource` + a Marten `PersistenceAdapter`. Plus a `json` primitive for open-shape data. Also: header-syntax reconciliation (all aggregate config on the header, paren modifiers, nothing in the body) which amends D-RENAME (rename `inheritanceStrategy` → `inheritanceUsing`, colon→paren) and relocates the shipped body `persistenceStrategy:` to a bare header `eventSourced`. Drops the per-containment hint; rejects "document as aggregate peer". Requests **D-DOCUMENT-AXIS**. |
