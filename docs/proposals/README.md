...
| [`channels.md`](./channels.md) | PROPOSED | **The transport tier for events** — fills the "async messaging/outbox" gap named in `production-readiness.md`. A context-member `channel { carries / delivery / retention / key }` (many per context; owns a subset of *its* context's events, like a `view`) unifies pub/sub, work-queue, and durable stream via orthogonal `delivery`×`retention` knobs. The contract names no transport: `channelSource` binds it to a `storage{type: redis/kafka/rabbitmq}` (the channel(domain)/channelSource(system) split mirrors `persistedAs(eventLog)`/`dataSource` per [D-STORAGE-SPLIT](../decisions.md#d-storage-split--split-the-overloaded-storage-keyword)), and browser realtime is derived from a UI `.live` subscription with the SSE-vs-WebSocket wire chosen by platform (overridable on the *deployable*, never the channel). Per-tenant / per-user fan-out is handled by **subchannels (rooms)** keyed by **two different things**: *visibility* = the subscriber's `DataKey` (tenant/org reachability from `authorization.md`, JWT-pinned, *reused not redefined*) as the room namespace + a subscribe-time admission check; and *interest* = the **React Query key** the page's `.live` binding already names (`byId(42)`→`["orders",42]`, `all`→`["orders"]`). So a detail page gets one aggregate, not the customer firehose, and a browser only receives what its claims permit. The interest key is deliberately the **same** key the React Query client, the realtime room, and the future server cache (`production-readiness.md` §3.4) all use — the **magic-caching** link: one `EventInvalidationIR` (event→query-keys) map, three consumers. Reuses the pinned `on(e: Event)` / `projection` consumer surface from `bounded-context-model.md` (reactor body = workflow body). Producer `emit` unchanged — the existing `DomainEventDispatcher` seam is the implementation point. |
