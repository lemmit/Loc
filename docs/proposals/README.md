...
| [`embedded-frontend-composition.md`](./embedded-frontend-composition.md) | PROPOSED | Isolates a modelling defect: a backend can express *that* it embeds a UI but not *which* frontend — the framework is **derived from the host platform**, not chosen. `dotnet`+`ui:` is hardwired to React at all three layers (generator calls `generateReactForContexts` unconditionally `dotnet/index.ts:274`; `expectedFrameworkFor` returns `react` for `dotnet`+ui `platform-rules.ts:94`; `Framework` enum has only `react`/`phoenixLiveView`). So `dotnet`-embeds-Angular, `phoenix`-embeds-React, `dotnet`-embeds-Vue are **inexpressible, not just unimplemented**. Distinguishes good encapsulation (`react` hiding Vite+router+react-query) from the actual leak (`dotnet` hiding `react`). Three options: **C** a frontend-generator registry making `framework:` *select* not *validate* (smallest, fixes the framework axis only) → **A** hosting as a property of the existing `targets:` edge (target model, fixes both framework + hosting seams, generalises to every backend). Success test: adding `dotnet`-embeds-Angular should need an Angular generator and **zero** changes to .NET hosting code. Mirror of the Ecto note's API-only finding; both converge on `react/index.ts:48–52`. |
