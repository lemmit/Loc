...
1. **ES-on-vanilla tail (P4.4)** — ✅ done: the T2.c returning-op tail (guard/emit
   ...
   (the union-find absent variant 404s uniformly across all six backends — Phoenix/Ash
   migrated off its 200 inline-tag) both shipped.  Residual: the `resource` extension
   on .NET's absent body (a `ControllerBase.Problem` limitation).
