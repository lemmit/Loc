...
| T3.9 | **Dapper/MikroORM beyond minimal-v1** — expansion in progress on Dapper (maintainer-prioritised): **capability filters shipped** (non-principal `filter` predicates splice into every SELECT's WHERE; principal ones stay gated). Remaining dapper gates: stamps, nested parts, associations, inheritance, document/embedded shapes, seeds, managed-access fields. MikroORM: decision still needed (expand vs document-as-minimal). | dotnet, node | [storage-and-platform-config](./storage-and-platform-config.md) |
