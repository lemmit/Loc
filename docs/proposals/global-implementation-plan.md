...
| T2.c | **Operation `or`-union returns on elixir** — ✅ **SHIPPED on `vanilla`** (#1208). `validateOperationReturnsUnimplemented` is foundation-aware (`elixir+vanilla` accepted, `elixir+ash` still gated); the context fn returns a tagged `{:ok, value} \| {:error, tag, data}` tuple and the controller translates each variant to its status (RFC-7807 ProblemDetails / 200), reusing the shared `util/error-defaults` resolution. v1 lowers `let`/`return` bodies (return-only, matching the cross-backend spike). **Remaining:** `precondition`/`emit`/`assign` inside a returning op, success-path serialization, a conformance-parity case, and the **union-find absence producer** (`validateUnionFindShapes` still exempts elixir-only hosts — the P4d tagger is success-side only; align it with the node/dotnet absence translation and drop the exemption). | ~~elixir~~ vanilla tail | [exception-less](./exception-less.md) / [vanilla-phoenix-foundation](./vanilla-phoenix-foundation.md) |
