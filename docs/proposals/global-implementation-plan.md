...
| T2.c | **Operation `or`-union returns on elixir** — ✅ **SHIPPED on `vanilla`** (#1208). `validateOperationReturnsUnimplemented` is foundation-aware (`elixir+vanilla` accepted, `elixir+ash` still gated); the context fn returns a tagged `{:ok, value} \| {:error, tag, data}` tuple and the controller translates each variant to its status (RFC-7807 ProblemDetails / 200), reusing the shared `util/error-defaults` resolution. v1 lowers `let`/`return` bodies (return-only, matching the cross-backend spike). The **custom-find HTTP surface** (#1217, `GET /<plural>/<find>`) and the **union-find absence producer** (#1218 — single-get + tagged success + absent→ProblemDetails; `validateUnionFindShapes` is now foundation-aware, exempt on Ash only) also shipped on vanilla. **Remaining:** `precondition`/`emit`/`assign` inside a returning op, success-path serialization, a cross-backend *wire*-parity case (the compile-parity fixtures already ship). | ~~elixir~~ vanilla tail | [exception-less](./exception-less.md) / [vanilla-phoenix-foundation](./vanilla-phoenix-foundation.md) |
