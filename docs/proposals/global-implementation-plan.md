...
| T3.14 | **React frontend remainders** — **find-filter list UI SHIPPED** (scaffolded list pages grow a filter bar for repository finds with all-string params + plain list returns: per-param bound input, `match`-driven all↔find switch, object-shaped `use<Find><Agg>({…})` hoist; `match` now renders in body position — chained ternary — and walker `==`/`!=` map to `===`/`!==`). Remaining: frontend-ACL Phase 3 (flat-key schema restructure + per-action FieldMaps — the plan doc's own deferral note says pull it when flat ≠ nested has a concrete consumer), richer filter inputs (enum select, paged finds, numeric). **Multi-segment / compound state assignment shipped** (nested-spread immutable updates — `order.shipping.zip := v` → `setOrder({ ...order, shipping: { ...order.shipping, zip: v } })`; unlowerable roots still fail loud). **`DestroyForm { of: <Agg> }` shipped** (canonical-destroy confirmation: window.confirm → `useDelete<Agg>` with the route id → navigate to the list; `then:` override; named destroys `for: <inst>.<name>` are a follow-up). | react | [retrieval](./retrieval.md), [loom-forms](./loom-forms.md), [frontend-acl](./frontend-acl.md) |
