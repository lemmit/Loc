...
| T3.2 | **Criterion selectability tail** — **`when <pred>` + auto-exposed `can-<op>` SHIPPED on all five backends (Hono + .NET + Python + Elixir/Ash + Java/Spring)** (409 Disallowed gate + side-effect-free `GET /{id}/can_<op>`; the Java service loads the aggregate, evaluates the predicate over its record accessors — enum → `<Enum>.<Value>` — throws DisallowedException → 409 ProblemDetail before mutating, and the controller returns `CanResponse { allowed }`; build-gated by `when.ddd` on both phoenix and java). `loom.when-unsupported` is now latent. Remaining: `from <Criterion>(args)` binding, `Repo.findAll(criterion, …)`, `private workflow`. | rides on reified criteria (T2.g) | [criterion](./criterion.md) |
