...
| A3 | expressible-builtins | concern | Self-inflicted overload: `onWrite precondition` reuses the existing imperative `precondition` statement keyword for an unrelated declarative construct. | Renamed to `writeGuard`. |
| A4 | expressible-builtins | concern | Self-inflicted redundancy: `new` used as an alias of `this` — a second spelling (and a costly soft keyword) in the anti-redundancy proposal. | Dropped `new`; only `old` + `this`. |
