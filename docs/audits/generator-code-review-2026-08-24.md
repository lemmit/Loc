...
| F2 — emission *mode* in the shared seam | **OPEN** | A8's fix is still a per-arm branch (`java/render-jpql.ts:91` `if (ctx.principalAccessors)`), not a declared mode that refuses an out-of-vocabulary construct. Ledger `G2667-F2` |
