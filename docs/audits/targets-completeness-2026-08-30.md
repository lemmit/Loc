...
| P4 | 91 |
| P5 | 12 |
| kind: silent / honest / breadth / mission / stale-prose | 108 / 40 / 26 / 65 / 12 |
| confidence: proven / likely / suspected | 90 / 157 / 4 |
| class: faulty-fix / regression | 5 / 0 |
| size S / M / L | 100 / 101 / 50 |
| provenance: fleet1-only / fleet2-only / corroborated by both | 179 / 69 / 3 |
| claimed by an open PR | 49 |
| done / merged | 53 |
| conflicts | 10 |
...
|---|---|---|---|---|---|---|
| P0 ! | `F2-CB-C2-tph-filter-dropped-dotnet` | silent | prov | dotnet | S | .NET silently drops every static capability `filter` on an aggregate that participates in an inheritance hierarchy |
| P0 ! | `F2-CB-C3-tph-filter-node-wrong-table` | silent | prov | node | S | node renders an aggregate's capability filter against `schema.<aggregatePlural>` — a table object that does not exist under TPH |
| P0 ! | `F2-ELX-ESCAPE-FUNNEL` | silent/faulty-fix | prov | elixir | S | The Elixir `#{` escaping funnel is incomplete: four emit sites splice a `.ddd` string into Elixir code with bare `JSON.stringify` |
| P0 ! | `F2-ADP-1` | silent | prov | dotnet/dapper, dotnet/efcore | M | `ignoring *` drops the `policy { deny }` always-false sentinel on .NET (both adapters) — read-denied rows are served |
...
| P0 ! | `F2-ADP-5` | silent/faulty-fix | prov | node/drizzle | M | drizzle — the DEFAULT node adapter — enforces `writeScopeFilter` only on the relational shape; mikroorm (the adapter that got the fix) enforces all fo |
| P0 ! | `F2-ADP-8` | silent | prov | python, java | M | `writeScopeFilter` is a documented no-op on python's non-relational shapes and absent entirely on java's document/event-sourced shapes |
| P0 ! | `F2-CB-C11-tph-base-owned-child-crosstenant` | silent | prov | node, java, dotnet, python, elixir | M | `abstract … with tenantOwned` + `crossTenant` concretes is accepted and leaves a NOT NULL tenant column with no stamp and no filter |
| P0 ! | `F2-CB-C4-tph-filter-node-base-field-dropped` | silent | prov | node | M | node silently omits a capability filter whose predicate reads a field declared on the abstract base |
| P0 ! | `F2-SEED-TENANT-NULL` | silent | prov | elixir, node, java, python, dotnet | M | A `seed` row on a `tenantOwned` aggregate is stamped with a NULL tenant on all five backends, so no tenant can ever read it |
| P0 ! | `M-T3.15-B0-mask-launder` | silent | prov | validator, node, dotnet, java, python, elixir | M | B0 — a `mask unless` value still launders through `emit` into a projection row and is served in cleartext |
| P0 ! | `connection-secret-wiring` | silent | prov | system-compose, kubernetes/helm, node, dotnet, | L | `connection:` on a storage parses, validates and lowers — no emitter reads it (credential wiring silently inert) |
| P1 | `F2-ADP-2` | silent | prov | dotnet/dapper | S | `persistence: dapper` raw seed inserts into a schema its own DDL never creates — boot-time 3F000/42P01, reproduced on the shipped corpus fixture |
| P1 | `F2-ADP-4` | silent/faulty-fix | prov | dotnet/efcore, dotnet/dapper | S | `writeScopeFilter` on an event-sourced aggregate emits a .NET repository that does not implement the port member it declares (CS0535), on BOTH adapter |
| P1 | `F2-ADP-6` | silent | prov | node/mikroorm | S | MikroORM `raw()` fragments splice BARE column identifiers — a reserved-word column breaks every queryable intrinsic (`startsWith`, `trim`, `round`, …) |
...
| P1 | `F2-CB-C8-domainservice-op-body-node-import` | silent | prov | node | S | node's aggregate class calls a domainService without importing it (op-body site) |
| P1 | `F2-CFE-5` | silent | prov | react, vue, svelte, angular | S | `OperationForm { of: <Agg>, op: <op> }` emits `use<Op><Agg>(id ?? "")` against an UNBOUND `id` — even on a page whose route declares `:id` |
| P1 | `F2-CFE-6` | silent/faulty-fix | prov | svelte, react, vue | S | The "a bare OperationForm still compiles" fix reaches only ONE of the two OperationForm shapes — the by-name shape emits Svelte referencing three un-i |
| P1 | `F2-CFE-7` | silent | prov | feliz | S | Feliz `asChild` prefix heuristic wraps a user-component call in an F# string literal — the component's source is shown as visible text AND the nested  |
| P1 | `F2-CFE-8` | silent | prov | angular | S | Angular silently drops component CHILDREN (the `Slot {}` content) — `ngComponentOutlet` cannot project content, and no degradation comment is emitted |
| P1 | `F2-CFE-9` | silent | prov | flutter | S | Flutter's `Money` / `DateDisplay` / `EnumBadge` primitives do not null-guard an OPTIONAL field — two produce Dart that fails static analysis, the thir |
| P1 | `F2-EXPR-1` | silent | prov | node, python | S | `sum(λ)` result type loses `money` in the IR — node/python then emit decimal-path arithmetic on a Decimal object |
| P1 | `F2-EXPR-2` | silent | prov | node, java, elixir, python | S | `let m = coll.map(λ)` is typed `string` in the IR — every downstream collection op on `m` mis-renders on 4 of 5 backends |
| P1 | `F2-EXPR-4` | silent | prov | node | S | node: `distinct` / `contains` over a value-object collection uses reference identity — silent no-op dedupe and always-false membership |
| P1 | `F2-EXPR-6` | silent | prov | python | S | python: `long.divTrunc` renders `int(a / b)` — a float round-trip that is wrong beyond 2^53, on the one backend with exact integers |
| P1 | `F2-FFE-2` | silent | prov | react | S | React `derived` / `title:` useMemo dependency arrays miss every ref that is not a top-level binary/ref/call-arg — ternaries, member receivers and intr |
| P1 | `F2-FFE-5` | silent | prov | feliz | S | Feliz `parseUrl` arms are emitted in page-declaration order — a literal route declared after a sibling parameterised route is unreachable (dead F# mat |
| P1 | `F2-FFE-6` | silent | prov | phoenixLiveView | S | Phoenix/HEEx picks `+` vs `<>` for string concatenation from whether an operand is a string LITERAL, not from its type — `who + other` emits `(@who +  |
| P1 | `F2-FFE-9` | silent | prov | all | S | `loom.sub-primitive-misplaced` (and its siblings) print the `where` prefix twice in CLI output |
| P1 | `F2-MT640-SORT-DEAD` | silent | prov | elixir | S | M-T6.40 shipped as option (a) — the non-paged elixir list page now compiles, but its sortable headers are a no-op refetch (and the mission row still r |
| P1 | `F2-SEED-DATASET-NAME-COLLISION` | silent | prov | elixir, node, java, python, dotnet | S | Two seed datasets whose identifier-cased names coincide emit duplicate seeder functions — compile error on 3 backends, silent data loss on 2 |
| P1 | `F2-SEED-RAW-DOCUMENT` | silent | prov | elixir, node, java, python, dotnet | S | `seed raw` on a `shape: document` aggregate emits a column-per-field INSERT against the (id, data, version) jsonb table — on all five backends |
| P1 | `F2-VAL-1` | silent | prov | all | S | AST validator crashes on a partially-parsed `derived` — aborting the entire `context` check and dropping every other diagnostic in it |
| P1 | `F2-W-05` | silent | prov | node | S | node is the only backend that emits `.000Z` on every `datetime` — RS-4's canonical form and the .NET converter's own comment are both wrong about it |
| P1 | `F2-XB-2` | silent | prov | angular | S | The code-point `.length` fix never reached Angular: generated Angular forms validate length in UTF-16 code units |
| P1 | `F2-XB-3` | silent | prov | node, dotnet, java, python, elixir | S | A resource-op inside an aggregate `test { }` block escapes `loom.resource-op-outside-workflow` — and fails five different ways, exactly the class the  |
| P1 | `G2667-C2-money-array-no-roundtrip` | silent | prov | node | S | 08-17 register #2: money[] fields don't round-trip on node (drizzle string[] never hydrates to Decimal[]) |
| P1 | `M-T5.9-reserved-not-emitted` | silent | prov | validator, language | S | `loom.reserved-not-emitted` does not exist — parse-but-no-emit surfaces are still silent |
| P1 | `M-T6.18-gap3-criterion-arg-types` | silent | prov | node, dotnet, java, python, elixir | S | A criterion/policy-fn invoked from a DECLARATION site (`filter X(42)`, `from X(args)`, `where:`, `when`) has its argument TYPES unchecked — the last l |
| P1 | `M-T6.26-doc-put-presence` | silent | prov | elixir | S | A `shape: document` aggregate's PUT still accepts an omitted required field on Elixir — the one changeset the presence check does not reach |
| P1 | `elixir-grapheme-vs-codepoint-length` | silent | prov | elixir | S | Elixir counts GRAPHEMES where the spec it publishes (and the other four backends) count code points |
...
| P1 | `fixed-slot-arity-membership-incomplete` | silent | prov | react, vue, svelte, angular, feliz, flutter, p | S | `loom.page-primitive-extra-children` (new) covers only Stat / KeyValueRow / op-form Modal — every other one-slot value primitive still drops extra pos |
| P1 | `ir-warnings-invisible-in-cli` | silent | prov | cli | S | `ddd parse` and `ddd generate system` discard every IR-level WARNING they compute (17 of 18 codes) |
| P1 | `queryview-single-lambda-paramtypes` | silent | prov | react, vue, svelte, angular, feliz, flutter | S | `QueryView { of: X.Agg.all, single: true, data: row => OperationForm { row.<op> } }` drops the form on EVERY frontend — the lambda binding never reach |
| P1 | `F2-ADP-7` | silent | prov | dotnet/dapper, dotnet/efcore, java | M | A `.ddd` field named after a TARGET-LANGUAGE keyword emits uncompilable C# and Java — the host-identifier twin the M-T6.42/M-T6.43 quoting fixes left  |
| P1 | `F2-CB-C1-paged-nonrelational` | silent | prov | node, dotnet, python, elixir | M | `find … paged` on a non-relational aggregate (eventLog / document / embedded) emits a route built for the paged contract against a repository built fo |
| P1 | `F2-CFE-1` | silent | prov | react, vue, svelte, angular, feliz, flutter, h | M | `navigate(<Page>)` in a page `action` body — the only documented home for navigation — is broken on all 7 frontend targets (feliz hard-crashes codegen |
| P1 | `F2-CFE-2` | silent | prov | react, svelte, angular, vue | M | Two forms on one page emit duplicate `const`/class-member declarations (react, svelte, angular) or silently cross-wire the second form to the first ag |
| P1 | `F2-CFE-3` | silent | prov | react, feliz, flutter | M | `For` as the direct body of a `QueryView { data: }` lambda emits syntactically invalid source on React (object literal), Feliz (`yield!` outside a seq |
| P1 | `F2-CFE-4` | silent | prov | react, vue, svelte, angular, flutter | M | The `auto-paged-table` macro rewrites only the Table's own `rows:` slot, leaving every SIBLING reader of the same QueryView binding pointed at the pag |
| P1 | `F2-FFE-1` | silent | prov | react, vue, svelte, angular | M | `store` state initializers are dropped on react / vue / svelte (all four lifetimes) and reset to the type zero on angular `persist: url` |
| P1 | `F2-FFE-3` | silent | prov | flutter | M | Flutter registers `'/things/:id'` verbatim as a `MaterialApp.routes` key while navigation pushes the interpolated path — every detail-page link on flu |
| P1 | `F2-FFE-4` | silent | prov | feliz | M | Feliz: a user component invoked from ANOTHER component's body is emitted without the `model` / `dispatch` curried arguments — `dotnet fable` fails (FS |
| P1 | `F2-FFE-7` | silent | prov | react, vue, svelte, angular, feliz, flutter, p | M | An unrecognized NAMED argument on a walker primitive silently swallows the content it carries (`Card { title: … }`, `Tab { title: … }`) — on every fro |
| P1 | `F2-SEED-EVENTSOURCED` | silent | prov | elixir, java, dotnet | M | A `seed` row on an event-sourced aggregate is silently dropped on elixir (marker still written) and emits an arity-wrong `create(...)` on java + dotne |
| P1 | `F2-W-01` | silent | prov | elixir | M | elixir multi-word valueobject field always reads back null (camelCase jsonb key written, snake_case key read) |
| P1 | `F2-W-13` | silent | prov | node, dotnet, java, python, elixir | M | `httpStatus NotFound -> <code>` also silently retargets the objectStore blob-absence 404, which the shared table's own comment says stays literal |
| P1 | `F2-XB-4` | silent | prov | node, dotnet, java, python, elixir | M | Every non-assignment statement in a folded-projection `on(e)` body is silently dropped on all five backends — and a `let` its own assignment reference |
...
| P1 | `M-T1.15-nonstring-filter-finds-dropped` | silent | prov | react, vue, svelte, angular, feliz, flutter, p | M | M-T1.15 — a repository `find` with an enum / numeric / `X id` parameter is SILENTLY excluded from the scaffolded list filter bar |
| P1 | `elixir-malformed-path-id-422` | silent | prov | elixir | M | The malformed-`{id}` 422 alignment (#2652) measured and pinned four backends; `platform: elixir` was never measured and still cannot answer the 422 it |
...
| P2 ! | `F2-CB-C12-tph-polymorphic-base-reader-unfiltered` | silent | susp | node | S | The emitted TPH polymorphic base reader queries the shared table with no capability predicate |
| P2 ! | `M-T3.9-dotnet-audit-masked-snapshot` | silent | like | dotnet | S | .NET records audit before/after snapshots through the MASKED serializer — the stored trail depends on who performed the write |
...
| P2 | `F2-ADP-9` | silent | susp | dotnet/dapper | S | dapper binds a Dapper parameter the bypassed SQL no longer references |
| P2 | `F2-CFE-12` | silent | like | react, vue, svelte, angular, feliz, flutter, h | S | `Modal { open: <state>, <OperationForm child> }` degrades to a comment and drops the entire modal, on every target |
| P2 | `F2-FFE-8` | silent | susp | feliz, flutter | S | Flutter and Feliz persisted stores write a FLAT JSON blob under the same `loom.store.<Name>` key the JS frontends use for zustand's `{state,version}`  |
...
| P2 | `F2-XB-7` | silent | like | elixir, system/wire-spec | S | `lineage` is declared a non-nullable object in `.loom/wire-spec.json` and in elixir's OpenApiSpex schema, while every backend puts `"lineage": null` o |
| P2 | `G2646-open-elixir-precondition-messages` | silent | like | elixir | S | #2646 documented, NOT fixed: authored `precondition` messages lost on the elixir raise path |
| P2 | `G2667-C7-heex-button-icon-loading` | silent | like | elixir | S | 08-17 register #7: Button { icon: / loading: } dropped on HEEx |
| P2 | `G2667-C8-heex-tab-slug-spaces` | silent | like | elixir | S | 08-17 register #8: HEEx tab slug keeps spaces (snake(label)) and diverges from JSX's slugify |
| P2 | `G2667-D2-provenance-leaf-blindness` | silent | like | node, dotnet, java, python, elixir | S | Debt: provenance leaf collection blind to convert/duration/list/i18nFormat/variant arms |
...
| P2 | `G2667-D4-dotnet-explicit-route-id-coercion` | silent | like | dotnet | S | Debt: .NET explicit-route id coercion contradicts the id wire mapping |
...
| P2 | `G2667-D7-walker-flag-propagation-divergence` | silent | like | react, vue, svelte, angular, feliz, flutter | S | Debt: propagateSinkFlags is a diverging near-copy of propagateChildFlags; stmtIsAwaited doesn't recurse as documented |
| P2 | `G2667-D9-i18n-slot-asymmetry` | silent | like | react, vue, svelte, angular | S | Debt: i18n slot asymmetry — Stat extracts label+value, KeyValueRow only the label |
...
| P2 | `domainservice-nullable-load-ungated` | silent | like | node, dotnet, java, python, elixir | S | An optional `find …: Agg?` bound in a `domainService` body emits an unguarded deref on all five backends — the new handler gate and its workflow twin  |
| P2 | `unresolved-page-ref-named-args` | silent | like | react, vue, svelte, angular, feliz, flutter | S | `loom.unresolved-page-ref` (new) walks positional args only — the same unresolved-ref defect in a NAMED argument still passes clean, including the `un |
| P2 | `F2-W-03` | silent | like | elixir | M | elixir validation `errors[]` pointers can only ever be depth-1 — `/dims`, never `/dims/width` |
| P2 | `G2646-open-heex-in-component-degradation` | silent | like | elixir | M | #2646 documented, NOT fixed: HEEx forms/queries/uploads/table-controls INSIDE a component still degrade silently |
| P2 | `G2646-open-heex-layout-inert` | silent | like | elixir | M | #2646 documented, NOT fixed: HEEx layout primitives semantically inert (Grid ≡ Stack, bare divs); non-server-paged Table gets no pager; i18nFormat wra |
...
| P2 | `G2667-D8-pack-loader-global-handlebars` | silent | like | react, vue, svelte, angular | M | Debt: pack loader mutates global Handlebars state; the 'one pack per generation' comment is stale |
| P2 | `M-T1.1-table-filter-silent-drop` | silent | like | phoenixLiveView, react, vue, svelte, angular,  | M | `Table { filter: … }` is silently ignored on HEEx and on ANY server-paged table — no marker, no diagnostic |
| P2 | `M-T1.12-raw-field-aria` | silent | like | svelte, angular | M | M-T1.12 — form-field `aria-invalid`/`aria-describedby` missing on flowbite (Svelte) and all three Angular packs; primeng additionally has NO label↔inp |
...
| P2 | `M-T1.8-error-boundary-five-targets` | silent | like | svelte, angular, feliz, flutter, phoenixLiveVi | M | M-T1.8 — the root render-time error boundary ships on React + 2 Vue packs only; Svelte, Angular, Feliz, Flutter and HEEx have none (a thrown render wh |
...
| P2 | `domainservice-member-chained-repo-read` | silent | like | node, dotnet, java, python, elixir | M | A repo read used as a MEMBER RECEIVER (`Accounts.byHolder(h).balance`) is not classified `repo-read`, so the domain service is typed `pure`, no read p |
| P2 | `nested-errors-pointer-shape` | silent | like | java, elixir, dotnet, node, python | M | Nested `errors[]` JSON pointer diverges: java emits a non-RFC-6901 `/lineTotals[0].unitPrice`, elixir emits nothing for nested children |
| P2 | `provenanced-bare-read-in-page-body` | silent | like | react, vue, svelte, angular, feliz, flutter | M | Provenanced<T> (#2653, merged 2026-08-24) changed the wire shape of a `provenanced` field under every page-body read; only the scaffold macro was taug |
...
| P2 | `schemathesis-F11-int32-range` | silent | like | node, dotnet, java, python, elixir | M | F11/W11+W12 — an `int` body field publishes no bound while the column is Postgres `int4`, so a contract-conforming value 500s; explicitly deferred by  |
...
| P2 | `static-subpath-405-node-only` | silent | like | dotnet, java, python, elixir | M | The F8 static-sub-path 405 guard landed on the Hono emitter alone — a wrong verb on a static sub-path still answers the sibling `/{id}` route's 422 on |
| P2 | `F2-W-04` | silent | like | elixir | L | elixir stores a valueobject as one jsonb `:map` column and never validates its inner fields — accepts bodies the other four 422 |
...
| P3 | `F2-CB-C10-tph-stance-not-inherited` | honest | prov | validator | S | `loom.tenancy-stance-unmarked` tells the author to add a marker they already added on the abstract base — capability FIELDS inherit, the stance does n |
...
| P3 | `F2-W-07` | honest | like | java | S | java publishes a differently-named paged envelope for a DECLARED `find … paged` (`PagedWidgetResponse`) than the other four (`WidgetPaged`) |
...
| P3 | `G2646-open-feliz-honest-residue` | honest | like | feliz | S | #2646 documented, NOT fixed: feliz `.map(λ)` has no lambda seam; component-host `match await` gated; three of four newly-throwing sub-shapes still lac |
| P3 | `M-T1.20-collection-op-gate-misclassified` | honest | like | react, vue, svelte, angular, feliz, flutter, p | S | M-T1.20 — `loom.frontend-collection-op-unsupported` is a DELIBERATE permanent gate but is registered as a drainable `gap` |
...
| P3 | `M-T6.36` | honest | like | java | S | Java emitter shape gaps — two narrow honest rejections still live |
| P3 | `F2-CFE-10` | honest | prov | react, vue, svelte, angular, feliz, flutter, h | M | String literals inside a `match` in a user-visible slot are silently untranslatable, while string `+` in the SAME slot is a hard validator error — `dd |
...
| P3 | `M-T1.10-handler-vocabulary` | honest | like | react, vue, svelte, angular, feliz, flutter, p | M | M-T1.10 — `on <channel>.<Event>` handler bodies remain a closed two-verb vocabulary (`toast` + `refetch`) |
| P3 | `M-T1.20-feliz-match-await` | honest | like | feliz | M | M-T1.20 — `loom.feliz-async-effect-unsupported` residue is narrower than documented: only a route-id-less host and a non-aggregate-instance subject st |
...
| P3 | `M-T5.3-nested-carriers-and-option` | honest | like | language, node, dotnet, java, python, elixir,  | M | Nested carriers stay gated; `option` lowers but the three-state PATCH it unblocks is unbuilt |
...
| P3 | `feliz-flutter-persist-codec-asymmetry` | honest | like | feliz, flutter | M | The new field-scoped `loom.store-lifetime-target-unsupported` splits feliz and flutter into two DIFFERENT covered type sets — a `persist:` store porta |
...
| P3 | `G2646-open-page-body-collection-ops` | honest | like | react, vue, svelte, angular, feliz, flutter, e | L | #2646 documented, NOT fixed: only `map` of 17 stdlib collection ops is usable in page bodies — the largest remaining page-DSL gap |
| P3 | `M-T1.3-keyed-folded-projection-reads` | honest | like | react, vue, svelte, angular, feliz, flutter, p | L | M-T1.3 — KEYED and FOLDED projection reads are unreadable from any frontend (honest gate, unclaimed) |
...
| P3 | `columnless-directtable-projection-emitters` | honest | like | node, dotnet, java, python, elixir | L | Direct-table query-time projections still cannot read event-log / document-jsonb / TPC-base storage on ANY backend — the universal gate is the whole a |
...
| P3 | `m-t4-8-contract-typed-resources` | honest | like | node, dotnet, elixir, python, java | L | M-T4.8 residual — the `contract` typed-resource declaration (inbound `from openapi(...)` clients) is unbuilt; `need ⊆ sourceType` activation is substa |
...
| P3 | `seed-tail-phases-5-7` | honest | like | cli, node, dotnet, elixir, python, java | L | M-T2.7 seeding tail — imperative workflow-body seed, `seed-spec.json` + compose seed step, and the `ddd seed` runner (+`--reset`, `key:` upsert) are a |
| P3 | `snapshot-policy-knobs-inert` | honest | prov | node, dotnet, elixir, python, java | L | M-T4.7 snapshots — `every:` / `retain:` on an `eventLog`/`snapshot` resource are read by zero emitters (warned, but the warning never reaches a CLI us |
...
| P4 | `M-T1.11-untranslatable-emitter-sentences` | mission | like | react, vue, svelte, angular, feliz, flutter, p | S | M-T1.11 — two emitter-built sentence frames remain untranslatable: the Chart accessible name and the ProvenanceInfo disclosure summary |
...
| P4 | `G2667-C-looseends` | breadth | like | react, vue, svelte, angular, test harness, ci | M | 08-17 loose ends still open: Card variant/shadow honoured by 3 of 15 packs (C9); branch-protection up-to-date rule (B4/M-T9.7); no emitter ever produc |
...
| P4 | `M-T1.3-scaffold-chart-tile` | mission | like | react, vue, svelte, angular, feliz, flutter, p | M | M-T1.3 Phase 5 — `scaffoldDashboard` emits an `<Agg>PerDay` series projection that NO scaffolded page reads; the Chart tile's stated blocker is gone |
| P4 | `M-T3.10-can-op-authz-aware` | mission | like | node, dotnet, java, python, elixir, react, vue | M | `can_<op>` is a bare `{allowed}` probe for `when`-gated ops only — no authz slice, no reason, no pendingValidation |
...
| P4 | `M-T3.15-C1-scaffold-binds-all` | mission | like | macros, react, vue, svelte, angular, feliz, fl | M | C1 — the scaffold's List page still binds `.all`, not a projection |
...
| P4 | `M-T9.33-firing-census-drain` | breadth | like | node, dotnet, java, python, elixir, frontends | M | M-T9.33 drain: 31 catalogued diagnostics still have no firing proof — chiefly the per-backend `*-unsupported` rejections M-T9.27's honest-gap guarante |
...
| P4 | `m-t4-10-backend-to-backend` | mission | like | node, dotnet, elixir, python, java | M | M-T4.10 backend-to-backend calls — the typed-invocation half shipped under M-T4.8; only peer authn / cross-stack peers remain, and no proposal owns th |
| P4 | `override-parity-non-node-routers` | breadth | like | node, dotnet, java, python, elixir | M | M-T9.25 round-2 probe 5: 'one override moves EVERY router' is asserted per-file on node only; the workflow/extern/projection routers are uncensused on |
...
| P4 | `M-T1.10-realtime-no-runtime-e2e` | breadth | prov | node, dotnet, java, python, elixir | L | M-T1.10 — the realtime SSE wire (incl. the security-relevant tenant-room routing) has NO runtime e2e on any backend; no cross-tenant isolation test ex |
| P4 | `M-T1.13-menu-reform` | mission | like | react, vue, svelte, angular, feliz, flutter, p | L | M-T1.13 — implicit sidebar derivation, the per-page `menu {}` bag, and the overloaded `menu` keyword all still ship |
...
| P4 | `M-T1.6-forms-tail` | mission | like | react, vue, svelte, angular, feliz, flutter, p | L | M-T1.6 forms tail — WizardForm, async refines, optimistic updates, option "leave unchanged", flat-key schema: no code for any of them |
| P4 | `M-T1.7-async-actions` | mission | like | react, vue, svelte, angular, feliz, flutter, p | L | M-T1.7 — `attempt {}` / `onError` / `spawn` / `async` do not exist in the grammar at all |
| P4 | `M-T1.8-errors-block-and-await-terminus` | mission | like | react, vue, svelte, angular, feliz, flutter, p | L | M-T1.8 — no `errors {}` declarative override and no unhandled-`await` terminus on any frontend |
...
| P4 | `M-T5.21-callable-unification` | mission | prov | language, node, dotnet, java, python, elixir,  | L | Fifteen callable productions, and the duplicated diagnostics they force, are all still there |
...
| P4 | `customization-cliff-study` | mission | like | react, vue, svelte, angular, feliz, flutter, h | L | Customization cliff — no study exists of what a no-code user hits when the ~55-primitive closed set runs out |
