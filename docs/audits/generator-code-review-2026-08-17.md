...
> **Implementation status (2026-08-18, this branch).** Every unclaimed finding below was implemented by an eleven-agent fleet on this branch, each fix mutation-proved and merged with the per-item evidence recorded in the merge commits: A1–A7, A9–A17 (A8 landed independently as #2596), B1/B2/B5 (B3 → #2599, B4 is an admin click), C1 (StmtTarget extracted, 392-tree byte-identity), C3–C8, C10–C12, D1–D3. §"Follow-up register" at the end lists the NEW findings the implementation itself surfaced — none are claimed anywhere yet.
...
## Follow-up register (2026-08-18) — new findings surfaced during implementation

Each was found by a fleet agent while fixing an audit item, verified in place, and deliberately left out of that fix's fence. None is claimed by an open PR at the time of writing.

1. **EF aggregation `ignoring` is silently not honoured.** The EF query-projection arm inherits `HasQueryFilter` correctly, but an aggregation carrying `ignoring <Cap>` never calls `IgnoreQueryFilters()` — the bypass the author asked for silently does not happen. Same class as the java half A1's fix closed. (Found fixing A1.)
2. **`money[]` FIELDS don't round-trip on node.** Drizzle maps the column to `string[]` while the domain holds `Decimal[]`; neither insert nor hydrate converts — `tsc` fails on the emitted repository. The `collection-op-shapes` fixture deliberately projects its money[] from a containment and records why in-file. (Found fixing A5.)
3. **Python audit-history route: pre-existing `F821`.** `current_user_ = current_user()` with no import in the generated audit-history route — fails the generated project's own ruff `F` rules. (Found fixing A15.)
4. **The mikroorm save path opens NO transaction at all** (`em.fork` + per-statement autocommit), so outbox atomicity (A3) structurally cannot hold there; fixing it means `em.transactional` around the save. Left because it overlapped #2597's territory. The TS document/embedded/eventsourced adapters have the same no-transaction shape. (Found fixing A3.)
5. **Workflow-step / extern-handler / timer emit sites insert outbox rows outside any transaction on all backends.** Symmetric, and there is no aggregate write to be atomic with — but whether a workflow's own state write should share a transaction with its emits is an undecided design question. (Found fixing A3.)
6. **~13 unescaped `data-testid="${testid}"` splices in `heex-primitives.ts`** — same quote-closes-the-attribute class C7 fixed for Column labels; author-controlled input, low severity. (Found fixing C7.)
7. **`Button { icon:/iconSvg:/loading: }` silently dropped on HEEx** — consistent with JSX packs that ignore unmapped knobs, but an untracked fidelity gap (the C9 "dead catalog" shape, now spanning `Card variant/shadow` on 12 of 15 JSX packs too). (Found fixing C7.)
8. **HEEx tab slug keeps spaces** — the anchor is `snake()`d, not slugified, so a multi-word caption yields `id="tabs-1-panel-overview label"`. (Found fixing C8.)
9. **`test/platform/packaging-split-{fs-discovery,core-pkg}.test.ts` fail in any git worktree** — they resolve `@loom/*` workspace symlinks that exist only in the primary checkout's `node_modules`. Every worktree-based agent hit it; worth a skip-with-reason or a symlink shim. (Found by three agents independently.)
10. **StmtTarget: three preserved per-backend inconsistencies**, each documented at its leaf with a `PRESERVED INCONSISTENCY (audit C1)` marker: temp-name numbering (positional vs per-kind), provenance gating (`emitProvenance` flag vs `prov`-presence + a dead `segments.length !== 1` guard), and node placing `value_computed` outside the provenance wrap where the other three put it inside. Byte-moving normalizations that each want their own gated change. (Recorded during C1.)
