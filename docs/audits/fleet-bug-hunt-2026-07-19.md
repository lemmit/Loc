...
| A2 | `avg(λ)` truncates on .NET; Java won't compile | wrong-value / build-break | **LIVE** — the desugar still stamps `leftType: numPrim` with no `rightType` (`lower-expr.ts:699-705`), so `isIntDivWidenedToDecimal` can't fire; operands still render `.Sum(int)` / `.Count()` |
| A3 | Python `%` is floored modulo | wrong-value | **LIVE** — `pyBinOp` returns `%` verbatim (`python/render-expr.ts:652-656`) |
| A4 | Java `money * int` / `money / int`, non-literal operand | build-break | **LIVE** — reproduced with the CLI: `derived unit: money = this.total / this.qty` emits `this.total().divide(this.qty(), MathContext.DECIMAL128)`; no such `BigDecimal` overload. `promoteMoneyOperands` is still literal-only (`literalPromotionAnchor`) |
...
**Tally: 12 fixed, 13 live** (E2 counted live at 3 of its 21 refs). Every live row keeps its original file:line + repro below — those were re-checked, not assumed. Drain order and grouping: **[M-T9.24](../new-plan/T9-toolchain-health.md)**.
