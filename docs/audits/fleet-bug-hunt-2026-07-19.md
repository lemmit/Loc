...
| F1b | Scaffolded money **form** RHF resolver input≠output | build-break | **FIXED 2026-07-30** — every pack's `useForm<…>` generic is now one context variable the shell computes, so the RHF THREE-generic form (`<FormState, unknown, Request>`) is emitted exactly where the schema reaches money and nowhere else. The workflows module had NO dual aliases at all, so a money-bearing `WorkflowForm` had no `FormState` name to reach for — added under the same gate. `tsc --noEmit` on generated projects: pre-fix TS2322 + TS2345, post-fix clean (mantine@v9 and shadcn@v4, create + update + workflow forms) |
| F2 | `match` in expression position emits `undefined` | wrong-value | **FIXED 2026-07-30** — `emitExpr` gained the arm, folding right into nested `exprTernary`s. NO new seam: `exprTernary` is already implemented by all SIX walker targets, so one arm in the shared core lands the fix on every frontend in its own idiom — JSX/Vue/Svelte/Angular ternary, F# `if/then/else`, Dart conditional. With no `else` the LAST arm becomes the tail rather than minting a per-language null literal |
