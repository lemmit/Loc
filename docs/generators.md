...
| User components | A walked `component <Name>(p: T) { body: … }` emits an F# function of its props record — `let <Name> (props: {| p: T |}) = …`, USED params re-bound as locals — spliced into `App.fs` ahead of the page views (F# is order-sensitive; Feliz has no per-component file), the channel the hoisted `DataGrid` child already rides (`component-emit.ts`).  The single anonymous-record parameter keeps ONE call form for both flavours: the `extern` seam's `<Name> {| … |}` already matches it.  An aggregate / value-object param types from the app's EMITTED wire records, so a record this app doesn't emit is never named.  Deferred (call site keeps the give-up comment): `state {}` / `derived` / named `action`s / reads / forms — a Feliz app is ONE Elmish program, so per-component state is an MVU design question (sub-models), not a rendering gap — plus a backstop that drops any component whose rendered F# reaches for `model` / `dispatch`. |
