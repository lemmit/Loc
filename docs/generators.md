...
| `test "…" { … }` | **Pure-subset ExUnit** (`src/generator/elixir/tests-emit.ts`, both foundations) → `test/<ctx>/<agg>_test.exs` (`use ExUnit.Case, async: true`) + a once-per-project `test/test_helper.exs`. Elixir has no pure in-memory factory (Ash actions / Ecto changesets validate against a live DB), so only an in-memory test — value-object construction (`%<Ctx>.VO{…}` on `ash`, a plain map on `vanilla`) + field reads asserted via `expect(x).<cmp>(y)` → `assert <x> <op> <y>` — emits as a runnable `test`. A test that calls aggregate `create`/operations, or asserts a construction-time invariant via `expect(…).toThrow()`, emits as an `@tag :skip` placeholder (name + reason preserved, never silently dropped) pending a DB-backed `mix test` harness. See [`docs/audits/test-parity-generated-backends.md`](audits/test-parity-generated-backends.md). |
