...
| `persistedAs(eventLog)` + `apply(...)` (event sourcing) | **deferred — IR-validate error** (`validateEventSourcedStorage`; `EVENT_SOURCING_BACKENDS` excludes Phoenix). The bar is *idiomatic Ash code*, and no idiomatic path expresses Loom's pure explicit-`emit`/`apply` model: [AshEvents](https://hexdocs.pm/ash_events) is **semantically incompatible** (auto-logs *actions*, no custom event types, no applier fold, replays by re-running actions, keeps a state table); [AshCommanded](https://hexdocs.pm/ash_commanded) *does* match (Commanded `apply/2` fold) but is a separate ES framework with its own EventStore (not `AshPostgres`-idiomatic) — a deliberate heavyweight choice, out of scope. A hand-rolled fold module would compile but isn't idiomatic Ash, so it's ruled out. Full rationale in [workflow-and-applier.md](proposals/workflow-and-applier.md). |
