...
| `aggregate Name [ids guid\|int\|long\|string] [persistedAs(eventLog\|state)] [normalised(true\|false)] { … }` | Aggregate root with implicit `Name id` field.  Header modifiers (D-DOCUMENT-AXIS): `persistedAs(…)` picks the primary truth kind (default `state`); `normalised(false)` marks the aggregate's saving shape as one JSON document (default `true` = normalised tables; document *emission* is a later slice). |
