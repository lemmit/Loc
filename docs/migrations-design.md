...
| TS/Hono | `src/generator/typescript/emit/migrations.ts` | **New**. Emit `db/migrations/<timestamp>_<name>.sql` raw SQL. Drizzle's migrator accepts raw SQL via `drizzle-orm/postgres-js/migrator` with the existing `journal` format. Don't run `drizzle-kit generate`; we own the SQL. Add prestart hook in Dockerfile / `package.json` `prestart` so `npm start` runs `npm run db:migrate` first |
| .NET | `src/generator/dotnet/emit/migrations.ts` | **New**. Emit `Migrations/<Timestamp>_<Name>.cs` with `[Migration("<ts>_<name>")]` class and `Up(MigrationBuilder b)` / `Down` bodies using `b.Sql(@"...")` raw SQL — avoids EF's model-snapshot tooling entirely. Also emit a minimal `ModelSnapshot.cs` stub. Suppress `RelationalEventId.PendingModelChangesWarning` in `OnConfiguring`. Add `Database.Migrate()` call in `Program.cs` startup |
