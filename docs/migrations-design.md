...
| .NET | `src/generator/dotnet/emit/migrations.ts` | **New**. Emit `Migrations/<Timestamp>_<Name>.cs` with `[Migration("<ts>_<name>")]` class and `Up(MigrationBuilder b)` / `Down` bodies using `b.Sql(@"...")` raw SQL — avoids EF's model-snapshot tooling entirely. **No `ModelSnapshot` stub is emitted**: the snapshot only feeds `dotnet ef migrations add`, which Loom never runs (the generator owns the source of truth). With no snapshot class in the assembly, EF's `PendingModelChangesWarning` has nothing to compare against and stays silent — no `ConfigureWarnings` suppression needed. `Database.Migrate()` in `Program.cs` startup consults the migration classes + `__EFMigrationsHistory` at runtime |
