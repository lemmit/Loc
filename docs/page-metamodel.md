...
| `Modal { trigger, … }` | Disclosure surface — hosts an `OperationForm` (scaffold detail pages) or a state-controlled `open:` body. The state-controlled form ships on **React, Vue, Svelte, Angular, Feliz and HEEx**; using it on Flutter is a compile error (`loom.modal-unsupported-target`), because that pack ships no dialogs and the modal's whole body would silently not render. `title:` is a user-visible slot on both shapes — it is the dialog's title, translated through the message catalog. |
