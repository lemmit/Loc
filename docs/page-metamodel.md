...
| `ProvenanceInfo(of:, field:)` | A "?" disclosure over a `provenanced` field's lineage (a native `<details>`/`<summary>`; [provenance.md](provenance.md)). Reads the co-located `<field>_provenance` wire sibling; scaffolded onto a provenanced field's detail row. Renders on React + Vue + Svelte; Angular/HEEx/Feliz render a comment (the value shows without the "?") until backfilled (M-T1.19). |
