...
- **query-time `projection`s** — the same optional `requires` gate, declared on
  the projection HEADER (`projection X keyed by k requires <expr> { … }`, after
  `keyed by`, like every other gate in the language), evaluated against
  `currentUser` before the read; failure → 403.  It is the projection twin of the find gate,
