...
export function renderStdlibMarkdown(): string {
  ...
  out.push(
    ...
    "",
    "**Backend-only.**  The frontend body walker has no renderer for these, so",
    "using one in a page / component expression is an error",
    "(`loom.frontend-collection-op-unsupported`) — compute the value in a",
    "repository `find`, an aggregate `derived`, or a `projection` read model and",
    "bind the result in the page.  The one exception is `map`, which the",
    "frontends render natively (`Array.prototype.map` / `Enum.map/2`).  See",
