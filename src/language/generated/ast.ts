...
export type Platform = 'dotnet' | 'elixir' | 'fastapi' | 'hono' | 'node' | 'phoenix' | 'phoenixLiveView' | 'python' | 'react' | 'static' | string;

export function isPlatform(item: unknown): item is Platform {
    return item === 'dotnet' || item === 'hono' || item === 'node' || item === 'react' || item === 'static' || item === 'phoenixLiveView' || item === 'phoenix' || item === 'elixir' || item === 'python' || item === 'fastapi' || (typeof item === 'string' && (/"(\\.|[^"\\])*"/.test(item)));
}
