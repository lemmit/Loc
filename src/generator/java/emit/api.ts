...
export function renderApiExceptionAdvice(
  ...
  return lines(
    ...
    `import ${basePkg}.domain.common.ExternHandlerException;`,
    ...
    `    }`,
    ``,
    `    @ExceptionHandler(ExternHandlerException.class)`,
    `    public ResponseEntity<ProblemDetail> onExternHandler(ExternHandlerException e, WebRequest request) {`,
    `        // A user-supplied extern handler threw — log the real cause server-side`,
    `        // via the catalog extern_handler_threw event (naming the seam) and return`,
    `        // a sanitized 500, distinct from a generic internal_error.`,
