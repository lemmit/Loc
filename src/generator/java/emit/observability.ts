...
import { javaLogEvent, javaLogEventLevel, javaLogEventName } from "../../_obs/render-java.js";
...
export function renderMigrationCatalogCallback(basePkg: string): string {
  return lines(
    ...
    `            switch (event) {`,
    `                case BEFORE_MIGRATE -> CatalogLog.event(${javaLogEvent("migrationsStarting")});`,
    `                case AFTER_EACH_MIGRATE -> {`,
    `                    if (info != null) {`,
    `                        CatalogLog.event(${javaLogEvent("migrationApplied")},`,
    `                            "id", String.valueOf(info.getVersion()),`,
    `                            "name", info.getDescription(),`,
    `                            "duration_ms", info.getExecutionTime());`,
    `                    }`,
    `                }`,
    `                case AFTER_MIGRATE -> CatalogLog.event(${javaLogEvent("migrationsComplete")});`,
    `                case AFTER_EACH_MIGRATE_ERROR, AFTER_MIGRATE_ERROR -> {`,
    `                    if (info != null) {`,
    `                        CatalogLog.event(${javaLogEvent("migrationFailed")},`,
    `                            "id", String.valueOf(info.getVersion()),`,
    `                            "name", info.getDescription(),`,
    `                            "error", "migration failed");`,
    `                    } else {`,
    `                        CatalogLog.event(${javaLogEvent("migrationFailed")}, "error", "migration failed");`,
    `                    }`,
    `                }`,
    `                default -> {`,
    `                }`,
...
export function renderLifecycleCatalog(basePkg: string): string {
  return lines(
    ...
    `    public void onReady() {`,
    `        CatalogLog.event(${javaLogEvent("serverListening")});`,
    `    }`,
    ``,
    `    @EventListener(ContextClosedEvent.class)`,
    `    public void onClosed() {`,
    `        CatalogLog.event(${javaLogEvent("serverShutdown")});`,
    `        CatalogLog.event(${javaLogEvent("serverDrained")});`,
    `    }`,
    `}`,
    ``,
  );
...
export function renderRequestCatalogFilter(basePkg: string): string {
  return lines(
    ...
    `        CatalogLog.event(${javaLogEvent("requestStart")}, "method", request.getMethod(), "path", request.getRequestURI());`,
    `        try {`,
    `            chain.doFilter(request, response);`,
    `        } finally {`,
    `            var durationMs = (System.nanoTime() - startedAt) / 1_000_000.0;`,
    `            CatalogLog.event(`,
    `                ${javaLogEventName("requestEnd")},`,
    `                ${javaLogEventLevel("requestEnd")},`,
