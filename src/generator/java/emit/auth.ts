...
export function renderAuthFiles(
  ...
  out.set(
    ...
    lines(
      ...
      `    }`,
      ``,
      `    /** The request-scoped principal for the current thread, or null`,
      `     *  outside a request — the static entry a JPA lifecycle hook reads`,
      `     *  (an entity can't inject this bean) to persist a declared claim`,
      `     *  stamp (\`createdByRole := currentUser.role\`). */`,
