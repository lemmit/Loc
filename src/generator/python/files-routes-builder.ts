...
export function renderPyFilesRoutes(fn: string, module: string): string {
  return lines(
    ...
    "    if obj is None:",
    "        # RFC 7807, through the app's own responder — this used to answer",
    '        # `{"error": "not found"}` as application/json, a second error',
    "        # contract on a wire already committed to problem+json (and a",
    "        # different one again on each of the other four backends).  The",
    "        # status stays a literal 404: a bucket key is not an aggregate id,",
    "        # so this is not the remappable `NotFound` rung.",
    '        return problem(request, 404, "Not Found", "No stored object for that key")',
