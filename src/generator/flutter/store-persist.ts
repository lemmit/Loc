...
export function renderStorePersistRuntime(stores: readonly FlutterPersistedStore[]): string {
  ...
  if (web) {
    body.push(
      ...
      "  /// throwing at first build.  Accepts BOTH the bare JSON object a JS",
      "  /// frontend writes and the string-wrapped form the web",
      "  /// `shared_preferences` backend hands back.",
  ...
  return `${lines(
    ...
    "// in `stores.dart`.  Keys (`loom.store.<Name>`) and the decoded JSON shape",
    "// match the React / Vue / Svelte / Angular store builders, so the same blob",
    "// and the same URL round-trip across frontends.",
