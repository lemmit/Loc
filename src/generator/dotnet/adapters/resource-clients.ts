...
const rabbitmqDotnetAdapter: DotnetResourceAdapter = {
  name: "rabbitmq",
  nugetDeps: () => ({ "RabbitMQ.Client": "6.8.1" }),
  emitClientClass(resources, _stores, ns): string {
    ...
    for (const r of resources) {
      ...
      lines.push(
        ...
        `    private static IModel? _${cls}Channel;`,
        "",
        `    private static IModel ${cls}_Channel()`,
        "    {",
        `        if (_${cls}Channel is { IsOpen: true }) return _${cls}Channel;`,
        `        var factory = new ConnectionFactory { Uri = new Uri(${cls}Url) };`,
        `        _${cls}Conn = factory.CreateConnection();`,
        `        _${cls}Channel = _${cls}Conn.CreateModel();`,
        `        return _${cls}Channel;`,
        "    }",
        "",
        `    public static Task ${cls}_Enqueue(string message)`,
        "    {",
        `        var ch = ${cls}_Channel();`,
        `        ch.QueueDeclare(queue: "${r.name}", durable: true, exclusive: false, autoDelete: false, arguments: null);`,
        "        var props = ch.CreateBasicProperties();",
        "        props.Persistent = true;",
        `        ch.BasicPublish(exchange: "", routingKey: "${r.name}", basicProperties: props, body: Encoding.UTF8.GetBytes(message));`,
        "        return Task.CompletedTask;",
        "    }",
        "",
        `    public static Task ${cls}_Publish(string topic, string message)`,
        "    {",
        `        var ch = ${cls}_Channel();`,
        `        ch.ExchangeDeclare(exchange: "${r.name}", type: "topic", durable: true);`,
        "        var props = ch.CreateBasicProperties();",
        "        props.Persistent = true;",
        `        ch.BasicPublish(exchange: "${r.name}", routingKey: topic, basicProperties: props, body: Encoding.UTF8.GetBytes(message));`,
        "        return Task.CompletedTask;",
        "    }",
        "",
      );
