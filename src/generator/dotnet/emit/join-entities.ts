...
export function renderJoinEntityConfiguration(assoc: AssociationIR, ns: string): string {
  ...
  return (
    lines(
      ...
      "{",
      `    public void Configure(EntityTypeBuilder<${className}> b)`,
      "    {",
      `        b.ToTable("${assoc.joinTable}");`,
      `        b.HasKey(x => new { x.${ownerProp}, x.${targetProp} });`,
      `        b.HasIndex(x => x.${targetProp});`,
      `        b.Property(x => x.${ownerProp}).HasConversion(v => v.Value, v => new ${assoc.ownerAgg}Id(v));`,
      `        b.Property(x => x.${targetProp}).HasConversion(v => v.Value, v => new ${assoc.targetAgg}Id(v));`,
