...
function emitContext(
  ...
  out.set("appsettings.json", DOTNET_APPSETTINGS);
  out.set("appsettings.Development.json", DOTNET_APPSETTINGS_DEV);
  out.set("README.md", dotnetReadme(ns));
}

const DOTNET_APPSETTINGS = `{
  "Logging": {
    "LogLevel": {
      "Default": "Information",
...
function dotnetReadme(ns: string): string {
  return `# Generated DDD Application (.NET)

This project was generated by Loom from a \`.ddd\` source.  Don't edit
auto-generated files by hand — re-run the generator instead.
...
- \`Domain/\` — aggregates, parts, value objects, events, branded ids,
  domain exceptions and the \`IDomainEventDispatcher\` boundary.
- \`Application/\` — Mediator commands, queries, request / response DTOs.
- \`Infrastructure/Persistence/\` — \`AppDbContext\` and per-aggregate
  EF Core configurations (\`OwnsMany\` / \`OwnsOne\`).
- \`Infrastructure/Repositories/\` — repository implementations.
- \`Infrastructure/Events/\` — \`NoopDomainEventDispatcher\` (replace it).
- \`Api/\` — ASP.NET Core controllers; they map Request DTOs to commands
  and dispatch via \`Mediator\`'s source-generated \`IMediator\`.

## Mediator

This project uses [\`Mediator\`](https://github.com/martinothamar/Mediator)
(MIT, source-generated) — *not* MediatR.

## Domain events

\`Program.cs\` registers a no-op \`IDomainEventDispatcher\`.  Replace the
binding with an outbox-table writer or message-bus publisher to wire
domain events into your infrastructure.

(Generated for the \`${ns}\` bounded context.)
`;
}
