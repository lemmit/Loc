import type { AuthIR, DeployableIR, FieldIR, SystemIR, UserIR } from "../../ir/types/loom-ir.js";
...
export function emitAuth(args: AuthEmitArgs): AuthEmitResult {
  ...
  files.set(`lib/${appName}_web/controllers/auth_controller.ex`, renderAuthController(webModule));
...
// ---------------------------------------------------------------------------
// AuthController — GET /auth/me session probe.  Returns the verified
// principal the Auth plug stamped onto conn.assigns (the `auth: ui` frontend
// guard reads it); works for both the OIDC verifier and the dev stub.
// ---------------------------------------------------------------------------

function renderAuthController(webModule: string): string {
  return `# Auto-generated.
defmodule ${webModule}.AuthController do
  ...
  @doc "GET /auth/me — the verified principal, or null."
