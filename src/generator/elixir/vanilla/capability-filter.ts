...
// `not record.is_deleted`).  Principal-referencing predicates (tenancy,
// `currentUser.*`) are excluded here — they need the request actor threaded,
// which plain Ecto has no ambient for; the IR validator still rejects them on
// the vanilla foundation.  A `filter <Criterion>` reference inlines its
...
/** The aggregate's non-principal capability filters as a single Ecto predicate
 *  (conjoined with the infix `and`, each parenthesised), or null when it has
 *  none.  `contextModule` feeds the shared `renderExpr` (enum / type vocab). */
export function vanillaCapabilityFilter(agg: AggregateIR, contextModule: string): string | null {
