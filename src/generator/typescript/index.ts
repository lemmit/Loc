...
export function generateTypeScript(model: Model): Map<string, string> {
  ...
  out.set(".env.example", ENV_EXAMPLE);
  out.set("README.md", README_TS);
  return out;
}
...
const ENV_EXAMPLE = `# Copy to .env and adjust.
DATABASE_URL=postgres://postgres:postgres@localhost:5432/postgres
PORT=3000
`;

const README_TS = `# Generated DDD Application (TypeScript)
...
npm run dev                      # start the Hono server on PORT (default 3000)
\`\`\`

## Layout

- \`domain/\` — aggregates, parts, value objects, events, branded ids.
- \`db/\` — Drizzle schema and per-aggregate repositories.
- \`http/\` — Hono routes per aggregate; \`createApp(db, events?)\` is the entry.
- \`index.ts\` — boots the HTTP server.

## Domain events

The default \`createApp\` uses a no-op \`DomainEventDispatcher\`.  Pass your
own implementation to publish events to an outbox or message bus.
`;
