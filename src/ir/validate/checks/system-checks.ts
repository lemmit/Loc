  ...
  for (const dep of sys.deployables) {
    ...
    for (const ctxName of dep.contextNames) {
      ...
      for (const agg of ctx.aggregates) {
        ...
        // Event-sourced aggregates have NO relational storage — their contained
        // parts are folded IN MEMORY (the `<Agg>Fold` builds nested maps over
        // each part's wire shape, incl. part-in-part via `[]`-defaulted nested
        // containments; `fold-stmt-emit.ts`), so the "no child table" reasoning
        // below does not apply.  Allowed.
