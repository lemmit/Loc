 ...
 * level only.  Every `StmtIR` kind carries only expression children (a nested
 * lambda block is reached by descending into the child expression, not here),
 * so a single `visit` callback suffices.  Exhaustive + `never`-checked.
 */
export function walkStmtChildren(s: StmtIR, visit: (child: ExprIR) => void): void {
  switch (s.kind) {
    case "precondition":
    case "requires":
    case "let":
...
export function walkStmtExprsDeep(s: StmtIR, visit: (e: ExprIR) => void): void {
  walkStmtChildren(s, (c) => walkExprDeep(c, visit));
}
