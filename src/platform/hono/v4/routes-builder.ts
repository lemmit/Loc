...
export function buildRoutesFile(
  ...
  lines.push(`        },`);
  // create → 400 (DomainError, caught in `app.onError` below) +
  // 422 (Zod parse failure → ProblemDetails with `errors[]`, emitted by
  // the shared `defaultHook`).  Per the shared error matrix +
  // docs/proposals/validation-error-extension.md.
  lines.push(
...
function emitOperationRoute(
  ...
  // operation → 400 (domain) + 404 (aggregate not found) + 422 (input validation
  // failure → ProblemDetails with `errors[]`), per the matrix +
  // docs/proposals/validation-error-extension.md.
  out.push(
    `      400: { description: "Bad Request", content: { "application/problem+json": { schema: ProblemDetails } } },`,
  );
