...
export function renderTimerScheduler(
  ...
  return lines(
    ...
    "// into a signed 32-bit int, so two replicas contend on the SAME key and exactly",
    "// one fires each tick.  pg_try_advisory_lock is non-blocking: the loser skips.",
    ...
    "",
    "  // One tick: single-fire lock -> build the event -> dispatch -> release.  The",
    ...
    "      running = true;",
    "      try {",
    "        const lock = await db.execute(sql`SELECT pg_try_advisory_lock(${lockKey}) AS locked`);",
